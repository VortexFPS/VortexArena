...
public sealed class ServerNet : IDisposable
{
    ...
    private void FlushSounds()
    {
        ...
        if (n > 0)
        {
            // BroadcastPacket, NOT _transport.Broadcast — on the threaded path this runs on the sim worker and
            // the Godot ENet peer is main-thread-affine (the outbox funnel; see the cross-thread crash fix).
            BroadcastPacket(_eventWriter.WrittenSpan, reliable: false);
