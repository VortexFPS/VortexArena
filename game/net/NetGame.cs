...
/// play</b>. It composes the real Â§2 netcode pieces the way <see cref="NetLoopback"/> does, but driven by
...
///   <item>a <see cref="ClientWorld"/> render layer + <see cref="ClientEntityView"/> netâ†’render bridge so
...
///         exact basis math <see cref="PlayerController.UpdateCamera"/> uses (copied here â€” we never touch
///         that class);</item>
...
/// trace against â€” replicating the server's real map collision on a remote client is a later layer (see the
...
/// then self-connects a local <see cref="ClientNet"/> to <c>127.0.0.1</c> â€” the listen-server case where the
...
/// <c>--map</c> for a quick 0-bot local match) builds a listen-server NetGame. The no-net GameDemo demo is gone â€”
...
public sealed partial class NetGame : Node3D
{
    ...
    // a non-headless host. When null, _Process drives _server.Tick directly on the main thread exactly as today â€”
    // no thread, no lock â€” so the default path is byte-for-byte unchanged.
    ...
    private VortexArena.Net.Demo.DemoRecorder? _demoRecorder;   // active `record` sink on ServerNet.DemoSink
    private VortexArena.Net.Demo.DemoPlayback? _demoPlayback;   // active replay source on ServerNet.ReplaySource
    private string _replayDemoPath = "";        // non-empty â†’ this listen server boots in replay mode
    ...
    // â†’ render via the cached LoadModel node, FORCEMODEL parity) OR a true VFS miss (LoadModel also null â†’ fall
    ...
    // The full CSQC HUD panel set (weapon bar / ammo / kill-feed / centerprint / timer) on the net play path â€”
    ...
    private AssetLoader? _injectedAssets;        // shared process-lifetime loader from MenuState â€” persists model/sound/material/texture/shader caches across maps & servers; null (or cl_persist_asset_cache 0) â†’ build a fresh per-match loader
    ...
    private bool _viewmodelReloading;              // last-frame reload state (clip_load==-1) â€” play the reload anim on the rising edge
    private bool _weaponsPrecached;             // PrecacheWeaponModels ran once (warm the per-weapon asset caches)
    private bool _readyComplete;                // _Ready finished â€” _Process can run its full body (before this, fields are half-built)
    ...
    // The SHARED first-person view subsystem (zoom + FOV + chase/death cam + eye-contents) â€” the SAME component
    ...
    // Accumulated view angles in DEGREES, Quake convention (X = pitch down-positive, Y = yaw, Z = roll) â€” the
    ...
    // per-weapon "next allowed shot" clock â€” a client mirror of the server's per-slot ATTACK_FINISHED
    ...
    private float _hostJoinNextRetryMsec;        // [#44] next allowed `join` attempt (bounded retry â€” a blocked spawn no-ops)
    ...
    private bool _inputActive;                   // tracks the activeâ†’inactive edge to release held buttons (pause/console)
    private bool _nadeDarknessActive;            // tracks the 0â†’positive darkness-remaining edge so SND_BLIND plays once per blind onset

    // C2S impulse (QC usercmd.impulse): a one-shot weapon-switch/reload number a weapon bind set this frame.
    // Edge-triggered â€” SampleInput stamps it onto the next InputCommand then clears it, so it is sent (and
    ...
    // 0â†’>0 transition and arm the shared view's spawn-zoom on that edge. -1 until the first health snapshot.
    ...
    /// <summary>Runs a console command line for one-shot binds (weapon_next/reload/kill/â€¦). Wired by
    ...
    /// <summary>Raised with a line of server console output (a <c>clc_stringcmd</c> reply on a remote client) â€”
    ...
    public Player? LocalServerPlayer
    {
        get
        {
            ...
            // WS1 stage 3: enumerate the COW roster snapshot â€” this property is read from the render thread
            // (HUD mirrors, feeds) while the sim worker may be mutating the live List on a join/leave.
            foreach (Player p in _serverWorld.Clients.PlayersSnapshot)
                if (!p.IsBot)
                    return p;
            return null;
        }
    ...
    /// <summary>Send a console command line to the connected server (DP <c>clc_stringcmd</c>) â€” used by the
    ...
    /// <summary>
    /// Execute a host-console gameplay command against the in-process server world, returning its output â€”
    /// or null when there is no local world (pure client). WS1: this runs on the MAIN thread (console input)
    /// while the sim worker owns the world when sv_threaded is on, so it takes the sim gate for the duration â€”
    ...
    /// Every world-mutating verb the console can type (kill/kick/give/endmatch/vote/â€¦) must come through here,
    /// not through a bare Commands.Execute â€” that was the mid-combat crash class d991956 closed for transport.
    /// </summary>
    ...
    /// (no server) and for a listen host that is alone (with or without bots â€” bots aren't peers).</summary>
    public bool HasRemoteClients => _isListenServer && (_server?.ConnectedPeerCount ?? 0) > 1;

    /// <summary>
    /// Raised when this listen server must change level (DP <c>changelevel</c>) â€” carries (map, gametype,
    /// botCount, botSkill, campaignId, campaignIndex) captured from the live match so the host can reboot the
    /// listen server on the new map preserving the mode + bot fill, and â€” for a campaign auto-advance on win â€”
    ...
    // them with must track server time â€” but advance smoothly BETWEEN snapshots (a snapshot only lands every
    ...
    // clock but MUST NOT advance it again â€” doing so ran it at ~2x wall time between rebases, which (once the
    ...
    private const float MaxInputBacklog = 0.25f; // â‰¤ ~18 catch-up input ticks per frame
    ...
    // sends a burst of commands the server applies in one go â†’ a one-frame movement lunge + a reconcile snap. With
    ...
    //   MaxInputFrameDt â€” hard spiral-guard cap on one render frame's simulated time (DP: bound(cl_timer, .., 0.1));
    ...
    //   MaxSubStepDt    â€” split a long frame into <= this so one huge physics step can't tunnel collisions
    ...
    // PATH A render-clock RATE SLEW (r16 rubberband fix â€” supersedes the per-snapshot ClockSyncFactor STEP):
    ...
    // (session-20260710-182544: frame-time lag-1 autocorr +0.61) â€” the felt "movement rubberbanding".
    //   ClockSlewGain â€” rate deviation per second of clock error (err 33ms â†’ the 5% cap);
    //   ClockSlewMax  â€” playback-rate deviation cap (Â±5%: never perceptibly fast/slow);
    //   MaxClockResync â€” discontinuity threshold beyond which we hard-snap (respawn / level change / a
    ...
    // Slew-target offset behind the newest snapshot (pairs with the hard upper bound at LatestServerTime in
    // _Process). Frametime parity audit 2026-07-11: Base (cl_nettimesyncboundmode 5, cl_parse.c:3354) targets
    // cl.mtime[1] â€” the PREVIOUS snapshot's time, a full MEASURED interval behind newest, so the cushion grows
    // adaptively exactly when snapshot production runs late (the loaded listen server) and interpolation never
    // rides the f=1 clamp edge. cl_interp_cushion 1 (default) = that DP parity via _snapInterval; 0 = the r16
    // half-tick constant bias below (legacy, ~7ms from the edge).
    private const float InterpBias = 0.5f / 72f;
    // Measured spacing between the last two snapshots (DP cl.mtime[0] - cl.mtime[1]), clamped to [1 tick, 0.1s];
    // the cl_interp_cushion slew/step target offset. Seeded at one tick until two snapshots have arrived.
    private float _snapInterval = 1f / 72f;

    /// <summary>How far behind the newest snapshot the render clock aims (see <see cref="InterpBias"/>):
    /// the measured snapshot interval (DP boundmode-5 parity, default) or the legacy half-tick bias.</summary>
    private float InterpCushion() => _interpCushionCv ? _snapInterval : InterpBias;

    // Fix B: a render frame longer than this (a stall on the shared listen-server thread â€” GC / heavy map streaming,
    ...
    // Per-frame (variable-dt) input mode â€” cl_movement_perframe (DP-style: one command per rendered frame stamped
    ...
    // The DeltaTime stamped onto the next sampled InputCommand. Always the fixed TicRate now â€” BOTH input modes
    ...
    /// <summary>
    /// [T63] Configure this node as a DEMO REPLAY host (planning/specs/demo-replay-and-spectator.md Â§10): a
    ...
    public bool ConfigureReplay(string demoPath, string playerName = "player",
        VirtualFileSystem? vfs = null, VortexArena.Engine.Simulation.CvarService? cvars = null)
    {
        VortexArena.Net.Demo.DemoHeaderInfo header;
        try
        {
            header = VortexArena.Net.Demo.DemoFormat.ReadHeader(demoPath);
        }
        ...
        ConfigureListenServer(header.MapName, header.Gametype, botCount: 0, playerName: playerName,
            serverName: "VortexArena Replay", vfs: vfs, cvars: cvars);
    ...
    public override async void _Ready()
    {
        ...
        if (_vfs is not null)
        {
            ...
            // them. Map-coupled resources (the lightmap atlas + its surface materials) are NOT in these caches â€”
            // they live in the map render tree and are freed with the match â€” so sharing is safe. A fresh loader
        ...
        // gracefully when a stage runs longer than expected, instead of one "Loadingâ€¦" â†’ snap-to-1 jump.
        // (Async void so a TeardownGame mid-load just leaves this coroutine to fall off the tree-check below.)

        if (_isListenServer)
        {
            LoadingScreen?.BeginStage("Loading mapâ€¦", 0.30f, 4.0f);
            await YieldForLoadingFrame();
            if (!IsInsideTree()) return;

            StartListenServer();

            LoadingScreen?.BeginStage("Booting worldâ€¦", 0.42f, 0.5f);
            await YieldForLoadingFrame();
            if (!IsInsideTree()) return;
        }
        else
        {
            LoadingScreen?.BeginStage("Preparing clientâ€¦", 0.30f, 0.5f);
            await YieldForLoadingFrame();
            if (!IsInsideTree()) return;

            BootClientFacade();

            LoadingScreen?.BeginStage("Resolving serverâ€¦", 0.42f, 0.5f);
            await YieldForLoadingFrame();
            if (!IsInsideTree()) return;
        }
        ...
        LoadingScreen?.BeginStage("Setting up rendererâ€¦", 0.55f, 1.0f);
        await YieldForLoadingFrame();
        if (!IsInsideTree()) return;

        // Render layer + the netâ†’render bridge + a basic HUD/camera. Built regardless of mode so a connect that
        ...
        LoadingScreen?.BeginStage("Starting musicâ€¦", 0.62f, 0.3f);
        await YieldForLoadingFrame();
        if (!IsInsideTree()) return;
        ...
        LoadingScreen?.BeginStage("Precaching weapon modelsâ€¦", 0.82f, 3.0f);
        await YieldForLoadingFrame();
        if (!IsInsideTree()) return;
        ...
        LoadingScreen?.BeginStage("Precaching soundsâ€¦", 0.87f, 1.5f);
        await YieldForLoadingFrame();
        if (!IsInsideTree()) return;
        ...
        // opening seconds: its 5 stock-model builds + GPU-warms otherwise land during the first ~5-7s â€” exactly the
        // pre-match countdown + streaming-settle window the player spawns into â€” adding frame hitches right then (a
        ...
        LoadingScreen?.BeginStage("Connectingâ€¦", 0.90f, 0.5f);

        // Keep the cursor FREE during load: the per-frame reassert in _Process grabs it for mouse-look the frame
        // the local player spawns (gated on LoadingScreen going null), so there's nothing to capture here â€” and
        ...
        GD.Print(_isListenServer
            ? $"[NetGame] listen server on 127.0.0.1:{_port} (map '{_map}', {_gametype}, {_botCount} bots) â€” self-connecting."
    ...
    /// is currently in flight. No-op (synchronously completes) if the node is no longer in the tree â€”
    ...
    /// prediction then traces against â€” the listen-server property that makes local prediction exact.
    /// </summary>
    private void StartListenServer()
    {
        ...
        // direct ConfigureReplay already set _replayDemoPath. A replay simulates no match of its own â€” the
        // recorded entities are the content â€” so any inherited bot fill is dropped.
        ...
        else
        {
            ...
            if (!string.IsNullOrWhiteSpace(_map))
                GD.PrintErr($"[NetGame] map '{_map}' not found in the VFS â€” listen server runs on a flat floor.");
        }

        // --- the headless world + gametype. Pass the map's parsed entity lump so the server spawns the real
        //     spawn points + gameplay entities (jump-pads/items/doors) â€” without these a player would spawn at
        ...
        // Resolve the threading decision NOW â€” before the world is built â€” because it selects the CVAR-STORE MODEL,
        // and the worker/gate setup further below must agree with that choice (so it's decided once, here).
        //   â€¢ Default (sv_threaded 0): UNIFY â€” the world runs directly on the shared menu/console/client store
        //     (DP's one engine cvar table). A host's console `set g_balance_â€¦`/physics tweak is then live in the
        ...
        //   â€¢ sv_threaded 1 (the DEFAULT since 2026-07-11, non-headless only): keep a PRIVATE per-world store,
        //     isolated from the main-thread menu/console writes and fed one-way by the bridge below â€” the sim
        //     runs on a worker thread, so a shared mutable store would race. sv_threaded is read from the shared
        //     store (where the menu/console writes it; unset â†’ ON); with no shared store (the degenerate bare
        //     host) we stay single-threaded + private.
        bool wantThreaded =
            _sharedCvars is not null
            && (!_sharedCvars.Has("sv_threaded") || _sharedCvars.GetFloat("sv_threaded") != 0f)
        ...
        // catch-up so a hitch isn't followed by a frame running 16Ã— the sim â€” that second spike would miss
        ...
        // ticks, and every per-tick Ã— per-player multiplier (WeaponThink, the bot strategy-token goal-rating)
        // fires that many times in one frame â€” the measured `mp.weapon Ã—28` / `bot.think` spikes on catch-up
        // frames. 3 trims that multiplier ~25% (the backlog just drains over one extra frame â€” brief, invisible
        ...
        // campaign file â€” QC CampaignSetup set the same cvars before the map change. (Must be a pre-Boot
        ...
        // ONCE, here at boot (changing it mid-match is out of scope â€” a rehost/map-change re-reads it). The actual
        ...
        // (Â§13.5 â†’ WS1) DEFAULT ON since 2026-07-11 (user decision, after the WS1 rework): the worker owns
        // tick+ENCODE, the render thread never blocks on the gate on the common path (per-tick holds; the
        // sv.gatewait_ms counter measures the residue), and the full validation battery is clean (soaks,
        // mid-match remote joins, 2989 tests). `sv_threaded 0` restores the single-threaded drive (and the
        // UNIFIED cvar store â€” see the store-model comment above).
        ...
        // make the host auto-spawn â€” which as a side effect made spectating impossible for EVERYONE (and armed
        ...
        // runs `join` for the LOCAL player once its observer attaches, retrying until it spawns â€” remote clients
        ...
        // (xonotic-server.cfg sets sv_maxidle_playertospectator 0, was 60) instead of a post-load code Set â€” so an
        ...
        // local host out (CampaignHostAutojoin) AND let the [#44] host-join block above spawn it â€” at the shipped
        ...
        if (!string.IsNullOrEmpty(_campaignName))
        {
            ...
            //    (and a replay of an earlier level can't regress it â€” Level == g_campaign<id>_index gate).
        ...
        // so without this a console `set g_balance_â€¦` â€” or any server-cvar tweak â€” never reaches the live match.
        ...
        // BotPopulation fixcount (GameWorld.OnStartFrame â†’ Bots.ServerFrame, QC bot_fixcount) fills one bot per
        // frame from time 2.5 â€” and, critically, a faithful fixcount REMOVES bots above the bot_number target
        ...
        if (_botCount > 0)
        {
            ...
            // `--host --bots N` leaves it -1 = unspecified â€” writing the old implicit default here silently
        ...
        _server = server;
        // Answer server-browser getinfo probes so this host shows up in the LAN list (no master heartbeat â€”
        ...
        if (!wantThreaded)
        {
            // STOCK single-threaded sinks â€” byte-for-byte the pre-S5 wiring (exact synchronous return values, which
            // the console feedback + the removebots loop depend on). This branch is what the default path uses.

            // player chat (say) â†’ broadcast to everyone incl. the sender; server announcements (vote/ban/team) â†’ same.
            ...
            //       â†’ Commands.ChatToPlayer; wire it to the reliable per-peer svc_print so each routed recipient
            ...
            // fixcount doesn't re-add); the disconnect chain fires Bots.BotRemoved â†’ ForgetPlayer clears ServerNet's
            ...
            // so each must be DEFERRED (Callable.From...CallDeferred) exactly like MapChangeRequested â€” never tear
            ...
            // QC quit_when_empty â†’ localcmd("quit"): shut the process when the last human leaves at match end.
            cmd.QuitHandler = () => Callable.From(() => GetTree()?.Quit()).CallDeferred();

            // QC quit_and_redirect â†’ redirection_target: reconnect every client (here: the local player) to the
            ...
            // QC lastlevel â†’ localcmd("set lastlevel 0\ntogglemenu 1\n"): after the final campaign/last-level map
        ...
        else
        {
            ...
            // which the worker thread owns â€” route each through ServerNet.RunOnSimThread so it executes on the sim
            ...
            // snapshot â€” faithful to how a remote add/remove would behave. This branch is reached ONLY when
            ...
            // server tick), so each uses CallDeferred to post the Godot tree operation to the main thread â€” the
        ...
        // GameTypeVote_SetGametype: fires on the sim thread (ApplyGametypeSwitch â†’ DriveEndOfMatchMapFlow), so the
        // handler runs synchronously on that thread â€” no RunOnSimThread wrapper needed on either path.
        // These hooks are ALIASES (gametypes-server.cfg defines `sv_vote_gametype_hook_<name>` for every gametype,
        // empty by default), NOT registered server commands â€” so they live in the ConfigInterpreter's alias table
        ...
        // which resolves the alias (DP localcmd â†’ engine console â†’ alias expansion). Stock empty aliases are a
        ...
        // unit-test world (no cfg load) â€” the `?.` then makes this a harmless no-op, same as no alias defined.
        ...
        // alias-resolution path as the gametype hooks above â€” dispatch through LoadedConfig.ExecuteLine so the
        ...
        // only on a bare unit-test world (no cfg load) â€” the `?.` then makes this a harmless no-op.
        ...
        // target_speaker ACTIVATOR (BIT3) â€” QC soundto(MSG_ONE, â€¦): play a one-shot sound to the TRIGGERING
        ...
        if (wantThreaded)
        {
            ...
            // on the main thread â€” hand it the SAME gate so that scan can't race the worker's spawn/relink. Stays
            ...
            // The trace service keeps shared mutable scratch and reads the live areagrid â€” MAIN-thread traces
            ...
            // WS1 stage 3: the main thread's display/feed reads of the PRIVATE server store (music cdtrack,
            // endscreen cvars, slowmo autopauseâ€¦) run ungated against the worker's writes â€” swap the store to
            // its thread-safe backing BEFORE the worker exists. The unthreaded default path never pays this
            // (the plain-Dictionary backing measured 32% faster per tick at 4 players).
            _serverWorld!.Services.CvarsImpl.EnableConcurrentReads();
    ...
    // =====================================================================================
    //  Demo record/replay wiring (T62/T63 â€” planning/specs/demo-replay-and-spectator.md)
    // =====================================================================================

    /// <summary>
    /// Attach the demo seams to the freshly-booted listen server: in replay mode open the demo and install it
    /// as <see cref="ServerNet.ReplaySource"/> (build-parity gated â€” an incompatible demo is rejected with an
    ...
    private void SetupDemoHooks(Commands cmd)
    {
        ...
        if (IsReplay)
        {
            try
            {
                var playback = VortexArena.Net.Demo.DemoPlayback.OpenFile(_replayDemoPath);
                if (playback.Header.BuildParity != NetProtocol.BuildParity())
                {
                    GD.PrintErr($"[NetGame] demo '{_replayDemoPath}' was recorded by an incompatible build â€” refusing to misrender it.");
                    playback.Dispose();
                    _replayDemoPath = "";
                }
                else
        ...
        // Roster catch-up: clients (bots included) who connect after recording started â€” an sv_autodemo
        // recording begins at match start, BEFORE the bots join â€” are appended to the recorder's roster and
        ...
        // exist), so an sv_autodemo match is flagged Recording with no writer attached â€” catch up now.
        if (_serverWorld.Demo.Recording && _demoRecorder is null && !IsReplay)
            StartDemoRecording(_serverWorld.Demo.CurrentDemoName);

        cmd.Register("record", "record <filename> â€” record a server-side demo to demos/<filename>.xgd", ctx =>
        {
            if (_demoPlayback is not null) { ctx.Print("cannot record while playing back a demo"); return true; }
            if (_demoRecorder is not null) { ctx.Print($"already recording to {_demoRecorder.Path} â€” `stop` it first"); return true; }
        ...
        cmd.Register("stop", "stop â€” finish and save the demo currently being recorded", ctx =>
        {
        ...
        cmd.Register("playdemo", "playdemo <filename> â€” play back a recorded demo (.xgd)", ctx =>
        {
            ...
            VortexArena.Net.Demo.DemoHeaderInfo header;
            try { header = VortexArena.Net.Demo.DemoFormat.ReadHeader(path); }
        ...
        cmd.Register("demo_pause", "demo_pause â€” toggle demo playback pause", ctx =>
        {
        ...
        cmd.Register("demo_seek", "demo_seek <seconds> â€” seek demo playback to an absolute time", ctx =>
        {
            if (_demoPlayback is null) { ctx.Print("not playing a demo"); return true; }
        ...
        cmd.Register("demo_speed", "demo_speed <factor> â€” set demo playback speed (0.25Ã—â€“4Ã—)", ctx =>
        {
            if (_demoPlayback is null) { ctx.Print("not playing a demo"); return true; }
            float factor = ctx.ArgFloat(1);
            if (factor <= 0f) { ctx.Print($"demo speed is {_demoPlayback.Speed:0.##}Ã—"); return true; }
            _demoPlayback.Speed = factor;
            ctx.Print($"demo speed {_demoPlayback.Speed:0.##}Ã—");
            return true;
        });
    }

    /// <summary>Start recording the live match to <c>demos/&lt;name&gt;.xgd</c>: build the header (map/gametype/
    /// build-parity/roster) and attach a <see cref="VortexArena.Net.Demo.DemoRecorder"/> as the server's
    ...
    private void StartDemoRecording(string name)
    {
        ...
        try
        {
            ...
            var header = new VortexArena.Net.Demo.DemoHeaderInfo
            {
                BuildParity = NetProtocol.BuildParity(),
                TickRate = 1f / VortexArena.Engine.Simulation.SimulationLoop.TicRate,
                MapName = _map,
                Gametype = _gametype,
            ...
            // demo_keyframe_interval (spec Â§4: ticks, default 144 â‰ˆ 2s @ 72Hz) tunes file size vs seek latency;
            ...
            float intervalSeconds = intervalTicks > 0f
                ? intervalTicks * VortexArena.Engine.Simulation.SimulationLoop.TicRate
                : VortexArena.Net.Demo.DemoRecorder.DefaultKeyframeIntervalSeconds;
            _demoRecorder = VortexArena.Net.Demo.DemoRecorder.CreateFile(path, header, intervalSeconds);
    ...
    /// <summary>One demo roster entry for a live player â€” shared by the record-start roster snapshot and the
    ...
    private VortexArena.Net.Demo.DemoRosterEntry RosterEntryFor(Player p)
    ...
    /// duration â€” the file is now self-contained and playable). Also the <see cref="DemoControl.StopRecording"/>
    ...
    /// in <c>&lt;user://&gt;/demos/</c> (DemoControl's auto-names already carry a <c>demos/</c> prefix â€” strip it
    ...
    /// <summary>
    /// Wire the <see cref="SandboxMutator"/>'s host seams â€” the C# successor to the cross-file calls the QC
    ...
    /// routed via <c>g_sandbox</c> â†’ Commands.CmdSandbox â†’ HandleCommand) executes but every effect is inert.
    ...
    /// <see cref="_server"/> / <see cref="_serverWorld"/> on the threaded path â€” so the print/trace touch the
    ...
    private void WireSandbox()
    {
        ...
        // QC print_to(player, msg) â†’ sprint(player, ..): one console line to the issuing player.
        ...
        // cl_sandbox_clipboard â€¦`) is now live for the listen-server local host: SetClipboard sets the cvar
    ...
    /// the SAME <see cref="_sharedCvars"/> store the menu/client use â€” so for that recipient we set the cvar
    ...
    private void SetSandboxClipboard(Entity player, string cvar, string value)
    {
        ...
        // HandleCommand escapes the serialized object (\" ) for the QC stuffcmd `set cvar "â€¦"` console parse;
        ...
        // (plain Dictionary, read/written by console/menu every frame â€” only the server's PRIVATE store is
    ...
    /// <summary>
    /// Wire the <see cref="SuperSpecMutator"/>'s host seams â€” the C# successor to the engine calls the QC superspec
    ...
    /// flags can be set (the command surface routes via <c>superspec</c>/<c>autospec</c>/â€¦ â†’ Commands.CmdSuperspec
    /// â†’ HandleCommand) but the manual follow* switches have no roster to scan and every message prints nothing.
    ...
    private void WireSuperspec()
    {
        ...
        // goes through the notification CenterRaw channel directly inside SuperSpecMutator.Msg â€” no host seam.)
        ...
        // QC .crypto_idfp â€” the player's auth UID, used to key the remote options file. PersistentId is the
    ...
    /// to set a remote client's cvar â€” see <see cref="SetClipboard"/>).</summary>
    private sealed class SandboxObjectStore : SandboxMutator.IObjectStore
    {
        ...
        // QC stuffcmd(player, strcat("set ", cvar, " \"", value, "\"")) â€” pushes the serialized object into the
        ...
        // (no svc_stufftext channel) â€” that recipient is silently skipped, matching the documented residual.
    ...
    /// server's real map collision (that needs the map name in the handshake + a client-side BSP load â€” a
    ...
    private void BootClientFacade()
    {
        ...
        // Apply() is idempotent (MutatorBase.Added guard) â€” safe to call even if called again later.
        MutatorActivation.Apply();
    }

    /// <summary>
    /// Build the predicted <see cref="ClientNet"/> and connect. The local player is predicted by driving the
    /// shared <see cref="Movement"/> sim over a carrier entity (<see cref="EntityMovementStep"/>) â€” the same
    /// deterministic sim the server runs â€” and the input is sampled live each tick from WASD/mouse.
    /// </summary>
    ...
    private Entity? SpawnCarrier()
    {
        ...
        // mutates the SAME entity table the worker ticks â€” take the gate around the spawn when threaded so it can't
        ...
        // SOLID_NOT so it never enters the trace/solid set â€” on a listen server the real authoritative Player
        ...
        // prediction ghost â€” but for MOVEMENT it IS a SOLID_SLIDEBOX player and must clip the SAME content mask
        // the authoritative server Player does (SpawnSystem sets the real player SOLID_SLIDEBOX â†’ Solid|Body|
        ...
        // projectile's server-side .Owner, so the owner trace-exception can't protect it â€” and the CORPSE bit in
    ...
    // cl_bench_spectate: next server-time the bench camera re-evaluates (0.5 s cadence â€” cheap, and the
    // re-glue after a spectatee death only needs sub-second latency, not per-frame work).
    private float _benchSpectateNextThink;

    /// <summary>True while <c>cl_bench_spectate</c> holds the host as an observer camera â€” read where the
    ...
    /// living bot, so the capture camera experiences real gameplay â€” map traversal, combat, and first-person
    /// gunplay via the follow-cam (<see cref="UpdateCamera"/>) + the spectated viewmodel â€” instead of idling
    ...
    /// re-observed â€” ClientManager's SpectateCopy gate) or leaves; if the host somehow joined the match (a
    /// stray +jump), it is pulled back to observer â€” bench mode owns the camera by design. Runs inside the
    ...
    private void BenchSpectateThink()
    {
        ...
        // NOTE: no cl_bench_spectate read here â€” on the threaded path this runs on the SIM worker and the
        ...
        if (!host.IsObserver)
        {
            // The real observer transition (model hidden, weapons stripped, non-solid, autojoin disarmed) â€”
        ...
        if (host.Spectatee is not null)
            return; // glued â€” SpectateCopy tracks the bot each tick
        foreach (var p in clients.Players)
        {
            ...
            // (which drops the follower to free-fly) this re-attaches â€” usually to the same bot once its
    ...
    private void SetupRender()
    {
        ...
        if (_assets is not null)
        {
            ...
            _render.Assets = _assets.Assets;   // material facade â†’ textured world/vehicle models (ModelResolver path)
            // Remote players render as skeletal IQM models (models/player/*.iqm); non-player networked entities
            // (items/gibs/monsters/props) render their real model by name â€” MD3 via the frame-driven
            ...
            // VFS miss returns null (QC fexists-miss â†’ the entity keeps its own model). Receives the Entity so the
        ...
        if (_assets is not null)
        {
            ...
            // placeholder box/cylinder â€” the ModelLoader Func they support was never connected, so that path was
        ...
        // [W14b LI3] the server clock for the remote torso-action overlay â€” the networked Entity.AnimActionTime is
        // stamped on this clock, so the client derives the action play phase as LatestServerTime âˆ’ start.
        ...
        // attaches this later â€” once the server's map name arrives â€” via LoadClientMapFromServer. No-op until then.
        AttachWorldRender();
        ...
        if (_client is not null)
        {
            ...
            // (RC6) instead of the gray placeholder box â€” same v_ model the first-person viewmodel uses.
            _entityView.WeaponModelFactory = BuildWeaponWorldModel;
            AddChild(_entityView);

            // Surface decoded effects/sounds to the renderer as packets arrive: effects â†’ particles, sounds â†’
            ...
            // net id â†’ its live origin (the local player from prediction, remotes from their interpolated pose).
    ...
    /// server â€” <see cref="_bsp"/> is loaded in <see cref="StartListenServer"/>) and
    ...
    private void AttachWorldRender(CollisionWorld? sharedEffectsWorld = null)
    {
        ...
        // no geometry). Reuses the SAME BSP + gametype submodel filter the collision was built from â€” render and
        ...
        // SECOND time on the connect frame â€” both the prediction trace and these effect traces are main-thread, so
        ...
        // Splats clip against the RENDER triangles (DP's actual target) â€” marks roll over visible trim/patch edges.
        _render.Effects.SetDecalGeometry(_bsp);

        // Chunked-SDF collision field for modern particles (planning/particles-dual-system.md Â§A). Built only in a
        // modern-collision mode (cl_particles_modern 1/2) â€” mode 0 (the faithful default) needs no SDF, so the
        // default connect pays nothing here. The SDF generator queries the world on a background Task, and
        // CollisionWorld.Query mutates a per-instance mark epoch â€” so it must NOT share the instance the
    ...
    private void SetupMusic()
    {
        ...
        if (_vfs is not null && !string.IsNullOrEmpty(_map))
        {
            ...
            if (_vfs.Exists(mapinfoPath))
            {
                ...
                catch { /* mapinfo read failed â€” fall through to worldspawn fallback */ }
            }
        }
    ...
    private static string ParseMapinfoCdTrack(string mapinfoText)
    {
        ...
        foreach (string rawLine in mapinfoText.Split('\n'))
        {
                ...
                line.StartsWith("cdtrack\t", StringComparison.OrdinalIgnoreCase))
            {
                ...
                // Strip quotes if present (QC cvar_value_issafe check â€” we just take the bare value).
    ...
    private void SetupCameraAndHud()
    {
        ...
        // particular boot path didn't load the full client cfg tree (Register is idempotent â€” it never clobbers a
        ...
        // Seed the camera fov from the view's *0.75 frustum (fov 100 â†’ ~83.6Â° vertical), then UpdateView keeps it
        ...
        // like GameDemo does â€” on a layer BELOW the lightweight hudLayer so the crosshair/radar/scoreboard draw on
        ...
        // reticle even without a Player) and leaves NetHud un-suppressed as the fallback â€” so there's never a DOUBLE
        ...
        // HIDDEN so the pure-client baseline (no Player ever resolves â†’ UpdateFullHudPlayer's null==null early-out
        // never runs its body) leaves NetHud's crosshair as the sole reticle. The listen-server nullâ†’Player edge
        // then shows it.
        _fullHud.Crosshair.Visible = false;
        //
        // The full HUD's Timer panel IS now fed: the server pushes the global match clock (NetControl.MatchState â†’
        ...
        // `cmd minigame â€¦` lines (the QC minigame_cmd localcmd). All wiring is in-process â€” the menu/renderer
        ...
        if (_client is not null)
        {
            ...
            // line is the SERVER command ("minigame move <bits>") â€” SendStringCommand IS the clc_stringcmd
            ...
            // Chat (QC HUD panel #12): the port has no dedicated chat net channel â€” server chat is broadcast via
            ...
            // in-game console via ConsolePrint elsewhere, so chat shows in both â€” matching DP, where chat lands in
        ...
        // by Shell) â€” the analogue of QC localcmd, the SAME single channel a bound key uses (RunBoundCommand). A
        ...
        // clc_stringcmd path a console command takes â€” emit `cmd ons_spawn <x> <y> <z>` (Onslaught.HandleOnsSpawnCommand
        ...
        if (_mapRoot is not null)
        {
            ...
            // see-through SubViewport portal above). Client-only â€” it reads the SAME un-networked
        ...
        // NetHud has no scoreboard â€” so the per-player columns + team totals that ClientNet decodes
        ...
        // driving the demo playhead directly (local single-viewer case, demo-replay Â§9). Replay only; on its own
        // CanvasLayer above the HUD so the controls sit on top. Keybinds (space/â†â†’/[ ]/,.) are handled in the bar.
        if (IsReplay && _demoPlayback is not null)
        {
            var replayLayer = new CanvasLayer { Name = "ReplayUi", Layer = 6 };
            AddChild(replayLayer);
            _replayBar = new VortexArena.Game.Hud.ReplayControlBar { Name = "ReplayControlBar", Playback = _demoPlayback };
            replayLayer.AddChild(_replayBar);
        }

        // Screen-effects layer (damage red-flash + liquid tint) â€” the SAME reusable ViewEffects node T4 built for
        ...
        // Nade orb effect models (heal/ammo/entrap/veil/darkness orbs â€” the static field entities a nade spawns).
        ...
        // also the one ClientWorld exposes â€” one shared instance. Its ModelFactory mirrors
        ...
        // connect-as-observer window â€” the DP gfx/loading.tga + progress bar, replacing the old plain black
    ...
    /// client cfg tree (a bare CLI host / pure client). Register is idempotent â€” it never clobbers a value the
    ...
    private static void SeedViewCvars()
    {
        ...
        // cl_motion_trace (r16 rubberband diagnostic): per-frame CSV of the motion-timeline signals â€” render
        // clock error/slew, server ticks per frame, camera/predicted/remote-entity rendered speeds, prediction
        // error magnitude â€” so a felt motion wobble can be attributed to its actual carrier. Port-only debug.
        Api.Cvars.Register("cl_motion_trace", "0");
        // Spectator follow-cam angle smoothing half-life in seconds (port quality knob, NOT a Base cvar â€”
    ...
    /// <summary>
    /// Install or swap the first-person weapon model to match the networked active weapon â€” QC view.qc:305-332
    ...
    /// fire-feedback gate holds prediction through it â€” else a held trigger through a switch predicted the flash
    /// up to the raise time EARLY. Decays in <see cref="UpdateLocalFireFeedback"/>; negative = idle.
    /// </summary>
    private float _switchRaiseLeft = -1f;

    /// <summary>
    /// Client mirror of QC <c>wepent.rl_release</c> (armed on release/equip, cleared on each predicted shot) â€”
    ...
    private void EquipNetworkedWeapon()
    {
        ...
        // Hide the gun when holstered (id<0), dead (Health<=0), the event/death chase camera is active â€” QC
        ...
        // (QC SV_ChangeTeam â€” FFA-only recolor), a pure client sends the `color` command. Change-gated.
        PushLocalPlayerColor();

        // Player colormap + per-weapon glowmod (Base viewmodel_draw 302-321): tint the held gun to the local
        // player's packed shirt/pants colors â€” the FFA profile color, or the team color in teamplay (the server
        // forces clientcolors there) â€” and glow with the pants palette / wr_glow. Applied every frame (cheap â€”
        // SetPlayerColors early-returns when the color is unchanged) so a mid-match team swap / recolor / charge
        // change repaints.
        UpdateViewModelColors();

        // Reload animation (Base wframe WFRAME_RELOAD â†’ anim_reload). The port does not network the wframe temp
        ...
        // already host-derived, but a pure client / a spectatee has no LocalServerPlayer slot â€” so drive the local
        ...
        // (Mod_Q3BSP_LightPoint) â€” the port lit everything with one global sun + flat ambient, so guns read
        ...
        // switchdelay_drop while the holster slide plays, and only then install the new model + raise â€” QC
        ...
        // drop/raise timing), so without this hold the swap looked instantaneous â€” worst on the out-of-ammo
        ...
        // (id check: a same-weapon wr_viewmodel override change â€” the Tuba instrument swap â€” rebuilds immediately.)
        if (!hidden && _equippedWeaponId >= 0 && id >= 0 && id != _equippedWeaponId)
        {
            if (_switchDropLeft < 0f)
            {
                ...
                // 0 default) lets the switched-to weapon open fire the moment it raises â€” surprising, since the
                // player never chose it. Release the attack buttons (DP -fire/-fire2, exactly what Base's opt-in
                // cvar localcmds) until a physical re-press â€” but ONLY when the outgoing weapon is genuinely dry
        ...
        // Map the active weapon â†’ its first-person v_ model: W_Model = "models/weapons/" + v_*.md3 (all.qc:233/367).
        // Weapon.WorldModel is the bare "v_laser.md3", so prefix the directory; a missing model â†’ placeholder bar.
        ...
        // the keypress holster in RunBoundCommand). Confirmed switch â†’ cancels any pending holster auto-recovery.
        _viewModel.PlayRaise();
        // The server slot now runs the incoming weapon's raise think (can't fire until it lands) â€” hold fire
    ...
    /// <summary>
    /// Resolve + push the held view-model's team colormap tint + glowmod (Base <c>viewmodel_draw</c> 302-321 â†’
    ...
    /// <c>clientcolors</c> â€” Base <c>entcs_GetClientColors(current_player)</c>: the FFA profile color, or the
    ...
    /// native glow. Cheap to call every frame â€” <see cref="ViewModel.SetPlayerColors"/> early-returns when
    /// nothing changed.
    /// </summary>
            ...
            if (net is "vortex" or "vaporizer" || net.Contains("nex"))
            {
                ...
                if (charge is { } c)
                {
                    ...
                    if (glow.R + glow.G + glow.B <= 0.001f)
                        glow = pants; // charge disabled â†’ fall back to the pants palette color
                }
            }
        }
    ...
    private int LocalViewedClientColors()
    {
        ...
        // remote table â€” the old lookup MISSED for a pure client's own colors, so the first-person weapon never
    ...
    private void PushLocalPlayerColor()
    {
        ...
        {
            _lastPushedClColor = 0; // never chosen â€” leave the server default alone
            return;
        }
        ...
        // Neither ready yet (connecting) â†’ try again next frame.
    }

    private int _lastPushedClColor = int.MinValue;

    /// <summary>
    /// Drive the first-person reload animation from the networked clip state â€” the port's stand-in for Base's
    ...
    private void UpdateViewModelReloadAnim()
    {
        ...
        if (LocalServerPlayer is { } p && !p.IsDead && !p.IsObserver)
        {
            ...
            // NOT consumed on a listen host (UpdateViewModelAnimFromNet returns early â€” "the host path owns it"),
            ...
            // (weapon_thinkf(WFRAME_FIRE1) on every attack) â€” the faithful per-shot edge on the live slot is the
            ...
            if (!reloading && af > _viewmodelLastAttackFinished && _serverWorld is { } sw && af > (float)sw.Time)
                ...
                // machine does explicitly (w_ready â†’ weapon_thinkf(WFRAME_IDLE)). Needed because SOME fire
                // clips are authored LOOPING (h_hagar fire loop=1) â€” the end-of-clip idle recovery never
    ...
    /// <summary>Last seen slot-0 ATTACK_FINISHED â€” the per-shot fire-anim edge for the listen host (see
    ...
    /// DP-style grid shading â€” Mod_Q3BSP_LightPoint samples per entity; the viewmodel is what the player
    ...
    /// MODE_LIGHTDIRECTION formula per pixel â€” position-varying colored shading structure instead of the
    /// r13 flat modulate. Scale is DP's ABSOLUTE 1/128 (Mod_Q3BSP_LightPoint <c>stylescale</c>: grid byte
    /// 128 = 1.0, above overbrightens â€” with the r_model_light_gamma response this reads like DP, where
    /// the r13 self-calibrating normalization was a crutch for the linear pipeline compressing it), tunable
    /// via <c>r_model_light_scale</c>. No grid on the map â†’ the PBR + fill-light fallback.
    /// </summary>
    ...
    /// no local slot â€” so resolve the WATCHED player's per-player WepentView slice off the entity stream and let
    ...
    /// there is no <see cref="LocalServerPlayer"/> (pure client) â€” on a listen host the host path already owns it.
    /// </summary>
    private void UpdateViewModelAnimFromNet()
    {
        ...
        if (_client.SpectatingNetId != 0)
            watched = _client.SpectatingNetId;            // following another player â†’ their networked anim frame
        else if (LocalServerPlayer is null)
            watched = _client.LocalNetId;                 // pure client (no local slot) â†’ our own networked anim frame
        else
        ...
        // enters the remote table, so the pure-client "watched == self" lookup used to MISS every frame â€” the
    ...
    /// color, ramping 0â†’0.3Ã— up to the anim limit then 0.3â†’1.0Ã— above it. Uses the stock balance defaults
    ...
    /// overrides â€” tuba.qc:423). Returns the <c>v_*.md3</c> filename to substitute for the active weapon's static
    /// view-model, or <c>""</c> for no override. Tuba swaps its model by the currently played instrument
    /// (<c>tuba_instrument</c>): 0 â†’ <c>v_tuba.md3</c>, 1 â†’ <c>v_akordeon.md3</c>, 2 â†’ <c>v_kleinbottle.md3</c>.
    /// The instrument is read from the local server player's slot-0 weapon state (host path); a pure remote client
    /// has no slot state here so it keeps the default Tuba model â€” the networked exterior model still updates.
    /// </summary>
    ...
    /// switched to or picked up â€” the brief stutter on a never-seen-before weapon. Building each model once here
    ...
    /// covers anything the player later picks up, switches to, or sees another player carry â€” faithful to
    ...
    private async System.Threading.Tasks.Task PrecacheWeaponModelsAsync()
    {
        ...
        // is left to AssetLoader's lazy cache â€” first encounter takes a one-frame hitch and then it's
        // cached. The cost saving is real: a dm map averages ~9 distinct weapons; warming 24 is ~2.5Ã— more
        // mesh/material/texture work than this match will ever use. Mutators that REPLACE the loadout
        // (instagib/overkill) shrink this further. Always register muzzle offsets, though â€” that uses a
        ...
        // expected loadout â€” the extra load cost is hidden by the loading screen and it removes the 30â€“300 ms
        ...
        // this one stage. Yield every few weapons (not every one â€” the yield costs a frame each).
        ...
        // freeing them unrendered â€” building+freeing warmed only the caches, leaving each weapon's material
        ...
        foreach (VortexArena.Common.Gameplay.Weapon w in VortexArena.Common.Gameplay.Weapons.All)
        {
            ...
            // header-only MuzzleTag parser â€” cheap enough to do for EVERY weapon so server-side projectile
    ...
    /// impact / explosion doesn't stall the frame decoding its OGG (the report's 5â€“50 ms first-play cost), and
    /// the first sight of a player doesn't stall building its skeletal IQM + textures (20â€“150 ms). Precaches
    ...
    /// picks) stays lazy â€” the idle-warmup queue (S3) mops up the long tail. Yields so the loading bar ticks.
    /// </summary>
    private async System.Threading.Tasks.Task PrecacheCombatSoundsAndModelsAsync()
    {
        ...
        // _assets). The throwaway build node is freed â€” only the material/texture caches are the goal. Other
        ...
        // (perf Â§9.4) Hosting â†’ warm the PLAYER-MODEL roster NOW, under the loading screen, instead of letting it
        // stream in during play. The set isn't bot-specific: a bot added at RUNTIME (bot_number raised mid-match â€”
        ...
        // NOTE: a human bringing a NON-roster custom model still cold-loads on first sight â€” a warm-on-first-seen
        ...
        foreach (string m in models)
        {
            ...
            {
                // (hitch-fix Â§3, 2026-06-15) Keep the built node to RENDER it below instead of freeing it
                // unrendered. Building-then-freeing warmed only the texture/material CACHES â€” Godot compiles a
                ...
                // (measured t=23-27s, rest-dominated, "1 ubershader"). This is the exact bug Â§12.6-2 fixed for the
            ...
            // frame â€” the load-screen analogue of the streamer's per-frame budget on the live path.
            await YieldForLoadingFrame();
            if (!IsInsideTree()) return;
        }
        // Render the roster offscreen for a few frames so every player-shader pipeline variant compiles NOW (under
        // the loading screen), then free them â€” the textures/materials stay cached. Mirrors StartIdleWarmup's
    ...
    private void StartIdleWarmup()
    {
        if (_assets is null)
            return;
        // `set cl_idle_warmup 0` disables the background warm (A/B switch â€” the concurrent player-model parse was
        ...
        // The other stock player models â€” streamed (S1): the IQM parse + sidecar reads run OFF the main thread
        ...
        foreach (string model in IdleWarmPlayerModels)
        {
            ...
            _streamer.Request(
                ...
                // (Â§12.3-1) Stage the texture pipeline like the live path â€” the idle warm previously paid the
                // same multi-hundred-ms monolithic build; now its decode runs on the worker and each upload is
                // its own budgeted job. (Â§12.6) The built model then RENDERS offscreen for a few frames before
    ...
    /// overrides that REPLACE the loadout (instagib â†’ vaporizer only, overkill â†’ ok* set, nix â†’ all normal
    /// non-mutatorblocked weapons it rotates through, <c>g_weaponarena</c> â†’ the arena list). Falls back to
    ...
    private System.Collections.Generic.HashSet<string> ComputeExpectedWeapons()
    {
        ...
        // Pure client (we don't see the map's entities, mutators, or cvars locally yet) â€” fall back to
        ...
        //     a cycling weapon). Return EARLY â€” don't union with map entities. ---
        ...
        // physically there, but the carried loadout is the arena set â€” that's what we definitely render).
        ...
        if (!string.IsNullOrWhiteSpace(arena) && arena != "0" && !arena.Equals("off", System.StringComparison.OrdinalIgnoreCase))
        {
            ...
            // Arena weapons fully define the loadout â€” don't union with map pickups, but DO keep the blaster
        ...
        // The blaster is granted on spawn in essentially every stock gametype â€” always include it.
        set.Add("blaster");

        return set;
    }

    /// <summary>The muzzle-flash effect name for a weapon â€” the QC <c>m_muzzleeffect</c> attrib of each
    ...
    /// <summary>
    /// The muzzle-flash MODEL vpath for a weapon â€” the QC <c>m_muzzlemodel</c> attrib (each weapon's
    ...
    /// model (nexflash.md3) but their <c>m_muzzlemodel</c> attrib is <c>MDL_Null</c> â€” they attach NO model flash,
    ...
    /// <summary>
    /// Build the third-person world model for a carried weapon id (RC6 â€” wired into
    ...
    /// Returns null (â†’ placeholder) for an out-of-range id, no asset loader, or a model that won't load.
    /// </summary>
    ...
    /// so IQM/DPM props/items/monsters render their real model instead of the placeholder box. Null â†’ placeholder.
    /// </summary>
    ...
    /// a pickup â€” a synchronous SURFACE compile (the residual a RenderDoc capture pinned to the MD3-entity class).
    ...
    /// <summary>#19 auto-pause realized the Xonotic way â€” via the <c>slowmo</c> cvar (DP host_timescale; the exact
    ...
    /// client â€” which scales its input cadence by the REPLICATED slowmo â€” freezes prediction in lockstep (so
    ...
    /// <summary>
    /// Dispose the client + (listen) server transports and free the prediction carrier â€” idempotent. Call this
    ...
    public void Shutdown()
    {
        ...
        // writer buffers 128 lines â€” without this a short capture (or any session tail) is silently lost.
        ...
        if (_processCvarsHooked && _sharedCvars is not null)
            _sharedCvars.Changed -= OnProcessCvarChanged;   // same store-outlives-match reasoning (Â§11 R11)
        _minigame?.Dispose();
        ...
        if (_serverWorld is not null)
        {
            ...
            _serverWorld.Simulation.TickGate = null;                  // per-tick gating off with the worker gone
        }
        _simGate = null;
        _server?.Dispose();
    ...
    // =====================================================================================
    //  Per-frame drive (the heart: server tick â†’ client poll â†’ send input â†’ camera)
    // =====================================================================================

    // (Â§11 R11) The three cvars _Process consults every frame, cached so the hot path does zero dictionary
    ...
    // 0 = BASE-FAITHFUL â€” transmit gated at 1/cl_netfps with bounded redundancy (intermediate frames coalesce above
    // ~cl_netfpsÃ—redundancy fps, exactly like DP), 1 = EXACT â€” transmit every predicted frame so the server replays
    ...
    private bool _netInputTraceCv;         // net_input_trace: dormant net inputâ†’movement pipeline diagnostic (see _Process)
    ...
    private bool _smoothDtCv;              // cl_smoothdt: conditioned client-motion dt â€” default OFF, see ConditionDt
    ...
    private bool _frameGovernorCv = false; // cl_frame_governor: adaptive frame pacing (r16; default OFF â€” opt-in experiment)
    ...
    /// <summary>Read a float cvar with an unset-aware default (GetFloat can't tell unset from 0 â€” several
    /// DP mouse-accel defaults are non-zero: minspeed 5000, power 2, in_pitch Â±90).</summary>
    ...
    private void RefreshProcessCvars()
    {
        ...
        // cl_predictfire defaults ON: unset GetString reads "" â†’ treat anything but "0" as on.
        ...
        // cl_netclock_smooth defaults ON (unset â†’ on): treat anything but "0" as enabled.
        _netClockSmoothCv = (_sharedCvars?.GetString("cl_netclock_smooth") ?? "") != "0";
        _netInputTraceCv = (_sharedCvars?.GetString("net_input_trace") ?? "") == "1";
        // cl_movement_hitch_hold defaults ON (unset â†’ on): treat anything but "0" as enabled.
        _hitchHoldCv = (_sharedCvars?.GetString("cl_movement_hitch_hold") ?? "") != "0";
        // cl_smoothdt defaults OFF (unset â†’ off, note the "0" fallback): raw measured delta, Base-faithful.
        ...
        // cl_smoothdt_driftcap defaults ON (unset â†’ on): bounded drift ledger + wide hitch gate
        ...
        // cl_interp_cushion defaults ON (unset â†’ on): DP boundmode-5 full-interval slew target; 0 = half-tick bias.
        ...
        _netClockDp5Cv = (_sharedCvars?.GetString("cl_netclock_dp5") ?? "") == "1";
        // cl_frame_governor defaults ON (unset â†’ on): adaptive frame pacing; 0 = off (A/B).
        _frameGovernorCv = (_sharedCvars?.GetString("cl_frame_governor") ?? "0") == "1"; // default OFF (r16: a workaround, not the fix â€” the variance program is)
        // DP cl_movement (default 1 = client prediction) / cl_nolerp (default 0 = interpolate) â€” r16 A/B.
        ...
        // m_smoothdt defaults ON (unset â†’ on): yaw-channel dt conditioning; 0 = legacy raw application (A/B).
    ...
    public override void _Process(double delta)
    {
        ...
        EnsureProcessCvarCache();   // (Â§11 R11) hot-path cvar values are cached; refreshed on Changed

        // (motion trace v3) Wall clock at the TOP of the frame. Godot's `delta` is a start-to-start interval,
        // so only a start-of-frame stamp differences into the same quantity â€” the v2 column was sampled inside
        ...
        // [r16 rubberband FIX â€” the conviction after the full elimination matrix] Godot's delta is the
        ...
        // carries a motion error proportional to that variance â€” the felt wobble, at ANY average fps,
        ...
        // RAW dt â€” its accumulator integrates variance harmlessly. cl_smoothdt 0 = raw dt for A/B.
        float dt = ConditionDt(rawDt);
        FrameGovernor(rawDt); // adaptive frame pacing (r16) â€” see the method's rationale block

        // WS1 instrumentation: how long the RENDER thread spent waiting on the sim gate last frame (per-trace
        // ConcurrencyGate acquisitions â€” prediction/particle/crosshair traces). Read+reset here so the counter
        ...
        // SAME factor the server applies to its sim (ServerNet.StepWorld â†’ SimulationLoop.TimeScale), so the player's
        ...
        // NOTE: the server is still driven by the RAW dt below (it scales internally via TimeScale) â€” only the
        ...
        // [T63] Replay drive (the two-clock model, spec Â§3): advance the demo playhead by real time Ã— speed
        // BEFORE the server tick so BuildEntitySet injects the fresh recorded set (the sim/snapshot clock keeps
        // running at real time â€” pause freezes the entities, not the stream), then re-broadcast the recorded
        ...
        if (_demoPlayback is not null && _server is not null)
        {
            ...
            System.Collections.Generic.IReadOnlyList<VortexArena.Net.Demo.DemoEventPacket> demoEvents =
        ...
        // S5: when threaded, the dedicated worker (XG-ServerSim) owns ServerNet.Tick â€” the main thread must NOT
        ...
        // exactly as today.
        // #19 auto-pause: mirror Shell's solo-local pause onto the SIM the Xonotic way â€” the `slowmo` cvar (see
        ...
        // joining remote still surfaces â†’ Shell auto-releases). Runs BEFORE the tick so the value is live this frame.
        ...
        // against the entire rest of _Process â€” a long tick stalled the render thread at the Enter, and a long
        // frame stalled the sim (why sv_threaded never actually flattened frames). What replaced it:
        //   â€¢ transport: stage-2 inbound marshaling (main only Polls; the WORKER processes packets/connects at
        //     its step top) + the stage-1 worker-encode outbox â€” PumpTransportThreaded takes no gate on the
        //     common path;
        //   â€¢ world MUTATIONS from main (bench spectate, host auto-join, map-vote impulse, chat/console sinks)
        //     ride ServerNet.RunOnSimThread;
        //   â€¢ crash-capable read races closed structurally: CvarService â†’ ConcurrentDictionary,
        //     ClientManager.PlayersSnapshot (COW roster) for cross-thread enumeration, _playerNetIds â†’
        //     ConcurrentDictionary;
        //   â€¢ prediction/particle traces serialise per-trace via TraceService.ConcurrencyGate (waits are
        //     bounded by the WS-BOT tick budgets, p99 ~2.5ms);
        //   â€¢ the remaining display reads of live Player/world fields (HUD mirrors, feeds, carrier mirroring)
        //     are tolerated stale/torn-per-field â€” the SAME class of ungated read the standalone HUD panels
        //     (scoreboard/radar/NetHud) have always done on the threaded path. Wrong-for-a-frame display data
        //     self-corrects; the reconcile absorbs a rare odd prediction seed.

        // S5 (Â§13.5): when threaded, the worker runs ONLY the pure-C# sim â€” the MAIN thread services the Godot
        ...
        // throttled to 2 Hz). WS1 stage 3: it MUTATES server-side spectator state â€” run it on the sim thread
        // when threaded (RunOnSimThread is inline when not). Its 2 Hz throttle field is then written only by
        // whichever thread owns the mode â€” never both. The cl_bench_spectate read happens HERE on main for
        ...
        // server cvar (listen-server only â€” the cvar isn't networked, so it stays empty/kill for a pure client,
        ...
        // intermission flag â€” armed with a short delay (QC `autoscreenshot = time + 0.1`) so the frame grabbed is the
        ...
        // [T51] HUD feeds produced by the server-side mutators (host path only â€” a pure remote client has no
        ...
        if (_server is not null)
        {
            ...
            if (Mutators.ByName("itemstime") is ItemstimeMutator itm && itm.IsEnabled)
            {
                ...
                // sv_itemstime=1) an ALIVE player in a normal round sees NOTHING â€” only spectators (mode 1/2) and,
        ...
        // player (spectator) it now reads that player's per-player WepentView slice off the entity stream â€” the
        ...
        if (_client is null)
            return;
        // Fix B (cl_movement_hitch_hold, default on): flag a frame HITCH (the shared listen-server thread stalled â€”
        ...
        // as a STEP on each fresh snapshot â€” every step jolted the interpolation sampling clock, visibly
        // speeding/slowing all remote entities for a frame; with the listen server's tick catch-up making
        // LatestServerTime elastic under combat load, the steps oscillated in ~300-600ms waves â€” the felt
        // rubberbanding. Now the clock's PLAYBACK RATE is modulated continuously (Â±ClockSlewMax): the same
        // error converges over a few hundred ms while entity motion never deviates perceptibly from real
        // time â€” frame rate and sim tick batching fully decouple from interpolation speed. Snap only on
        ...
        // cl_netclock_dp5 (parity-audit A/B): 1 = Base's exact boundmode-5 STEPPED law, applied per snapshot
        // arrival below (snap >0.5s / 50% step >0.1s / Â±1..2ms creep), free-running at wall rate between
        // snapshots like DP's cl.time += clframetime. 0 (default) = the r16 continuous rate slew here.
        bool dp5 = _netClockDp5Cv && _netClockSmoothCv;
        float slew = 0f;
        if (_lastSeenServerTime >= 0f && _netClockSmoothCv && !dp5)
            slew = Math.Clamp((_client.LatestServerTime - InterpCushion() - _renderClock) * ClockSlewGain,
        ...
        // snapshot â€” sampling beyond the interp window clamps the two-snapshot lerp at f=1, rendering every
        // remote entity at its raw newest pose (uninterpolated tick-lumps: the felt combat rubberbanding; the
        // motion trace measured the clock ~9ms PAST latest on average). Hard upper bound here; the slew/step
        // above targets InterpCushion() behind (default: a full measured snapshot interval, DP mode-5 parity â€”
        // the DP-equivalent bound, since Base's target cl.mtime[1] also sits one interval below its window edge
        // cl.mtime[0]), so this clamp only engages a full cushion past target. Falling far behind still
        // hard-snaps via MaxClockResync below.
        ...
        if (_client.LatestServerTime != _lastSeenServerTime)
        {
            ...
            // Measured snapshot spacing (DP cl.mtime[0] - cl.mtime[1]): feeds the cl_interp_cushion full-interval
            // target, which is what lets the cushion GROW when the loaded listen server produces snapshots late.
            if (!firstSnapshot)
                _snapInterval = Math.Clamp(_client.LatestServerTime - _lastSeenServerTime, 1f / 72f, 0.1f);
            _lastSeenServerTime = _client.LatestServerTime;
            float err = _client.LatestServerTime - _renderClock;
            if (firstSnapshot || !_netClockSmoothCv)
                _renderClock = _client.LatestServerTime;
            else if (dp5)
            {
                // Base cl_nettimesyncboundmode 5, verbatim (cl_parse.c:3354-3361), against the cushioned target
                // (DP's cl.mtime[1]): snap on a discontinuity, halve big errors per snapshot, else creep at
                // -2ms/+1ms per snapshot (asymmetric: falls behind faster than it catches up â€” never overshoots).
                float target = _client.LatestServerTime - InterpCushion();
                float terr = _renderClock - target;
                if (MathF.Abs(terr) > 0.5f)
                    _renderClock = target;
                else if (MathF.Abs(terr) > 0.1f)
                    _renderClock += 0.5f * (target - _renderClock);
                else
                    _renderClock = Math.Clamp(target, _renderClock - 0.002f * slowmo, _renderClock + 0.001f * slowmo);
            }
            else if (MathF.Abs(err) > MaxClockResync)
                _renderClock = _client.LatestServerTime;
            // (no per-snapshot step otherwise â€” the rate slew above owns convergence)
            if (firstSnapshot)
            {
                ...
                // C5: the camera now has an authoritative owner state to sit at â€” start placing it (the per-frame
        ...
        if (_client.Accepted)
        {
            if (!_loggedAccept)
            {
                ...
                // A pure --connect client now knows the server's map â€” load its BSP so the remote player renders
            ...
            // [#44] The listen host's "Create Game and play" join â€” Base's `early_join_requested` arm of the
            ...
            // passive delayed-autojoin no longer exists (faithful â€” it only armed when sv_spectate was 0), so
            // without this the host â€” and every scripted flow that expects the host to spawn (--map smoke,
            // --camera-trace, perf runs, --screenshot's CaptureGate) â€” would sit at the observer prompt forever.
                ...
                && _serverWorld is { } joinWorld)
            {
                ...
                {
                    _hostJoinDone = true; // spawned (or no host player yet) â€” nothing to auto-join
                }
                else
                {
                    ...
                    if (nowMsec >= _hostJoinNextRetryMsec)
                    {
                        ...
                        // WS1 stage 3: the join MUTATES the world â€” run it on the sim thread (inline when
                        // unthreaded). The retry-until-spawned loop already tolerates the deferred execution:
                        // IsObserver stays true until the worker runs it, so we just retry/latch next frame.
                        Player? joinCaller = LocalServerPlayer;
                        if (_server is { } joinSrv)
                            joinSrv.RunOnSimThread(() =>
                                joinWorld.Commands.Execute("join", isServerConsole: false, caller: joinCaller));
                        else
                            joinWorld.Commands.Execute("join", isServerConsole: false, caller: joinCaller);
                        // If the join spawned us this frame (unthreaded path), IsObserver is now false â†’ latch.
                        if (LocalServerPlayer is not { IsObserver: true })
                            _hostJoinDone = true;
                    }
                }
            ...
            // trace set, and the carrier's slide-move (PlayerPhysics.PushEntity, MoveFilter.Normal) clips Body â€”
            ...
            // Only the host body is skipped â€” projectiles/world still clip â€” so the projectile-passthrough fix
            ...
            if (_carrier is not null)
            {
                ...
                // it runs no movement during the countdown â€” otherwise it predicts gravity/creep the frozen server
                // never applies, and the per-snapshot sub-32u error gets smoothed+re-armed every frame (the vibrate,
                // which then takes a freeze-length to bleed off â†’ looked like movement freed ~5s into live). Gate on
            ...
            // NET-INPUT DIAGNOSTIC (dormant; `set net_input_trace 1`). Logs the full inputâ†’movement pipeline every
            ...
            // drained into a movement batch; enet throttle (0..32 â€” gates UNRELIABLE sends, a low value silently
            ...
            // (the counters it reads are single increments on the hot path â€” negligible â€” so they stay always-on).
            ...
            // latched facing here â€” BEFORE the input loop below samples/sends the view â€” then clear (one-shot). A
            ...
            if (LocalServerPlayer is { FixAngle: true } fixSelf)
            {
                ...
                // (ConsumePredictedFixAngle) â€” the server's stamp for the SAME crossing lands here 1-3 frames
                // later (its tick runs behind the replay). Re-applying it would throw away every mouse delta
                // in between (a "view fights me" snap-back on each crossing), so recognise it â€” same facing,
                // recent â€” and only clear the flag. Spawn snaps and unpredicted (multi-dest) teleports, which
                ...
                // own view ONCE) â€” this is the listen-host equivalent of that single-apply.
                NVec3 fa = fixSelf.FixAngleAngles;
                // The AUTHORITATIVE stamp drives warpzone view snaps (the predicted apply is disabled on the
                // listen host â€” see the wz_predict_apply block above). Skip only when a predicted path (the
                // teleporter snap) already applied this same facing recently â€” the round-3 known-good rule.
                ...
                if (!predictedSame)
                {
                    ...
                    // this authoritative path â€” the predicted warp no longer stamps a view snap).
            ...
            // input tick â€” the felt "snap" on high-refresh displays. It also latches the press for the next sampled
            ...
            // Send model (Path A): EXACT = transmit every predicted frame (InputSendInterval 0 â†’ server replays the
            // identical sequence â†’ reconcile ~0); BASE-FAITHFUL (default) = gate transmits to cl_netfps/s, the
            // bounded redundancy then coalescing intermediate frames above ~cl_netfpsÃ—redundancy fps exactly as DP
            ...
            if (_perFrameInput)
            {
                ...
                // 1/72 tick boundary to alias against the display rate â€” that aliasing was the bhop "lurch" the
                ...
                // (half-step Verlet gravity â†’ dt-invariant apex; verified by MovementTimingTests Phase1), so variable
                // dt does not distort the arc; only the landing tick carries the tiny Â±dt quantization Base accepts.
                ...
                // flooding the server. _inputAccum is unused here (no tick draining) â€” zeroed so the legacy sub-tic
            ...
            else
            {
                ...
                while (_inputAccum >= tic && drained < MaxClientCatchupTicksPerFrame) // cap: graceful hitch recovery
                {
                    _inputAccum -= tic;
                    drained++;
                    // Sample WASD/mouse â†’ InputCommand â†’ push to the ring, predict forward, send the redundant tail.
                    ...
                    // advance _renderClock per drained tic here â€” that was the ~2x double-advance (see _renderClock).
                    _client.SendInput(_renderClock);
        ...
        // (NOT every frame â€” that would reset the asymptote and stall the bar) and dismiss it via
            ...
            if (stage != _handshakeStage)
            {
                ...
                switch (stage)
                {
                    case HandshakeStage.Connecting:
                        LoadingScreen?.BeginStage("Connecting to serverâ€¦", 0.94f, 1.0f);
                        break;
                    case HandshakeStage.WaitingForServer:
                        LoadingScreen?.BeginStage("Waiting for first snapshotâ€¦", 0.97f, 0.5f);
                        break;
                    case HandshakeStage.Joining:
                        LoadingScreen?.BeginStage("Joining gameâ€¦", 0.99f, 0.5f);
                        break;
        ...
        // Spawn-zoom edge (QC cl_spawnzoom): the net path has no Teleport, so derive (re)spawn from a Health 0â†’>0
        ...
        // while ATTACK2 is held and g_balance_vortex_secondary is 0 (the stock default â€” the networked active
        // weapon arrives as ClientNet.ActiveWeaponId). Suppressed when dead (QC cl_unpress_zoom_on_death â€” the
        ...
        // QC cl_player.qc CalcRefdef: `if(autocvar_chase_active) vieworg = CSQCPlayer_ApplyChase(...)` â€” the classic
        ...
        if (_viewEffects is not null && _client is { Accepted: true })
        {
            ...
            // Darkness-nade blind overlay (QC nade/darkness.qc â†’ STAT(NADE_DARKNESS_TIME) â†’ the CSQC full-screen
            // fade). The stat is the ABSOLUTE server time the blind expires; the client renders the remaining
            // window = NadeDarknessTime âˆ’ now. The host reads the live local Player; a pure remote client reads
            ...
            // SND_BLIND ("misc/blind"): a one-shot 2D cue on the 0â†’positive onset edge (QC plays it as the blind
        ...
        if (_shownamesLayer is not null)
        {
            ...
            // QC current_player + 1: the entnum of the player we are VIEWING â€” the followed spectatee when
            ...
            // spectatee_status != 0 â‡” the local client is not a live in-game player (free-fly observe OR follow).
            ...
            // [#55] The REAL teamplay flag (networked ScoreInfo â†’ GameScores.Teamplay; the in-process server
        ...
        // desired state every frame â€” SetWantCapture is idempotent (it only touches Input.MouseMode on a real
        // change) â€” rather than edge-latching on a remembered flag. The old latch desynced after a pause-menu
        ...
        // stays FREE (DP behaviour â€” you can alt-tab / mouse out of a windowed game while it loads). LoadingScreen
        ...
        // r16 rubberband diagnostic â€” last so it records this frame's FINAL camera/prediction state.
        MotionTrace(dt, rawDt, slew);

    }

    // ---- cl_frame_governor (r16): adaptive frame pacing â€” "reliable frame time while the rate varies" ---
    ...
    // no pacing the panel receives frames of wildly varying age â€” the felt wobble, at any average fps.
    // The governor paces frame starts at a rolling target just under what the scene currently sustains:
    // every second it sets Engine.MaxFps to ~0.98/p75(recent raw dts) â€” the limiter is then ENGAGED on
    ...
    private void FrameGovernor(float rawDt)
    {
        ...
        // chases itself downward (the first build spiraled 137->83). React instead to the OVER-CAP TAIL â€”
        ...
        //                  only smooths when DEEPLY engaged â€” at the edge, cheap frames sleep but expensive
        ...
        //   else        -> hold (deeply paced and sustainable â€” the desired steady state).
        int target;
        if (_govApplied == 0)
        {
    ...
    // ---- cl_smoothdt: conditioned client-motion dt â€” DEFAULT OFF since 2026-07-26 ----------------------
    ...
    // onto a physics_step/N grid with a Â±50 ms repayment ledger (planning/wobble-independent-audit-2026-07-26.md
    // Â§3f â€” the actual wobble), so the "raw" leg was never raw. It compared median-filtered-mangled against
    // mangled. With `physics_jitter_fix 0` restoring the measured delta, the release-export playtest found NO
    // felt difference between the legs â€” so the filter is carrying cost for no measured benefit:
    //   â€¢ it is the only remaining dt-side contributor to felt-band speed error (measured by the two-clock
    //     detector on a 311 s release capture: engine clock 0.01% RMS, after ConditionDt 0.46%);
    //   â€¢ it is AUTHORITATIVE, not cosmetic â€” the conditioned dt ships in InputCommand.DeltaTime, so the
    //     server integrates it: filter error is real movement-speed error (strafe timing, jump distance);
    //   â€¢ the driftcap sheds real wall time by design (that capture: 244 ms over 311 s, +0.078% slow);
    //   â€¢ Base has no dt filter at all, and Godot's previous-frame delta IS DP's cl.realframetime semantics
    //     (frametime-parity audit) â€” there is no lag here to correct that DP does not also have.
    ...
    // is not a median-of-9 but a DISPLAY-CADENCE estimator â€” snap to the measured refresh period with drift
    // repayment (the seam doc's S2, done with a correct refresh number; Godot misreads 60 Hz in borderless).
    //
    // What it does when enabled (1): replaces the motion path's dt â€” the CLIENT MOTION PATH only â€” with an
    // estimate of the CURRENT frame's duration rather than last frame's measurement:
    //   estimate = median of the last 9 raw deltas   (robust to one-off spikes â€” the classic frame pacer),
    //   hitches pass through RAW                      (a real stall must advance real time â€” no slow-mo),
    //   drift correction                              (a bounded Â±4% nudge keeps Î£(smooth) == Î£(raw), so
    ...
    private float _dtDrift;             // Î£(raw) - Î£(smooth): positive = smooth is BEHIND wall time
    ...
    private float ConditionDt(float rawDt)
    {
        ...
        // A real hitch (or a big cadence change) passes through raw â€” and resets nothing: the ring absorbs
        // it and the median follows if the new cadence persists.
        // [driftcap] The legacy 1.8Ã—/0.5Ã— gate MISSES the common cap engage/disengage transitions:
        // 144â†”250 fps is ratio 1.736/0.576, so 4-5 frames of the old cadence each dumped ~3 ms into the
        // ledger â€” a deterministic ~300 ms saturated-repayment episode per transition (2026-07-26 audit).
        // 1.6Ã—/0.6Ã— brackets those ratios; the cost is that 1.6-1.8Ã— jitter outliers now pass raw (rare).
        ...
        // this frame so total conditioned time tracks wall time exactly; Â±4% is imperceptible per frame.
        float bound = median * 0.04f;
        float nudge = Math.Clamp(_dtDrift * 0.25f, -bound, bound);
        float smooth = median + nudge;
        _dtDrift += rawDt - smooth;
        // [driftcap] Bound the ledger to 16 frames of full-rail repayment (0.64Ã—median â‰ˆ 4.4 ms @144fps).
        // Unbounded, a big excursion rides the Â±4% clamp until fully repaid â€” a SUSTAINED speed error the
    ...
    // ticks (server catch-up bursts), cam/pred speeds (the OWN view's rendered velocity â€” waves here =
    // own-movement rubberband), pred_err (reconcile corrections), remote_speed (one tracked remote entity's
    // rendered velocity â€” waves here with a steady cam = entity-interp rubberband).
    //
    ...
    // un-normalized signals instead â€” cam_step/yaw_step (what one displayed frame embodies), raw_dt (before
    ...
    //
    // v3 (engine-delta forensics): the wobble turned out to be measurable WITHOUT a display-side clock at all â€”
    // by comparing the delta the ENGINE reports against wall time over the same frames. Godot's
    // MainTimerSync::advance_checked rewrites process_step (clamp band from physics_step, Â±jitter_fix ledger),
    ...
    //   qpc_top_s â€” QPC sampled at the TOP of _Process, so diff() is the same start-to-start interval Godot's
    ...
    //   frame     â€” Engine.GetFramesDrawn(), so a SKIPPED row is detectable. Rows are skipped whenever dt <= 0,
    ...
    private void MotionTrace(float dt, float rawDt, float slew)
    {
        ...
        if (!haveRemote)
        {
            // Prefer a PLAYER entity (a bot in motion) â€” the first trace tracked the lowest id, which was a
            ...
            // NOTE (v2 fix): this used to reset _mtHave â€” which, with NO remote player in the session
        ...
        if (_mtHave)
        {
            ...
            // v2: raw per-frame steps (NOT divided by dt â€” see the header note) + the PresentMon join clock.
            // cam_step_xy: horizontal-only step â€” during a laser-jump/bhop flight arc the horizontal speed is
    ...
    private void UpdatePickupFeed()
    {
        ...
        if (_pickupInit)
        {
            ...
            bool pickedUp = false;
            {
                ...
                // Resource pickups â€” a per-frame jump above a threshold regen never reaches in one frame
    ...
    /// Reads the host-side <see cref="LocalServerPlayer"/>'s active weapon-slot scratch state â€” the C# home of
    /// QC's networked <c>wepent.*</c> fields â€” and maps it onto the panel's ring setters: Vortex charge
    /// (<c>vortex_charge</c> â†’ <see cref="CrosshairPanel.ChargeFraction"/>) + chargepool (<c>vortex_chargepool_ammo</c>
    /// â†’ <see cref="CrosshairPanel.ChargePool"/>); the reload/ammo ring (<c>clip_load</c>/<c>clip_size</c>); the
    ...
    /// the one that owns it, so the panel only draws the ring for the weapon currently held â€” exactly like the QC
    ...
    /// <c>ServerNet.ResolveOwnerWeaponRings</c>) â€” so a remote client draws the same rings the host does.
    /// </summary>
    private void UpdateCrosshairWeaponRings()
    {
        ...
        // view-state (charge/clip/heat) off the entity stream and drive the same crosshair rings â€” previously a
        ...
        // owner-block scalars (ServerNet.ResolveOwnerWeaponRings â†’ OwnerWeaponRings, read by ClientNet). The
        // server already resolved each ring's -1/0 'no data' sentinel for the held weapon, so just mirror them â€”
        ...
        {
            // Live host owner but no live weapon (dead / observing) â†’ hide every ring (sentinels match the
    ...
    private void UpdateVehicleHud()
    {
        ...
        // source the panel from the networked VehicleViewState instead â€” the remote pilot's own-entity slice, or
        ...
        // Not piloting (on foot / observing / dead) â†’ the hud_id == HUD_NORMAL exit case (hides + clears aux).
        ...
        // Mirror BOTH reload bars (QC 0.01 * STAT(VEHICLESTAT_RELOAD1/2)) â€” the NE bar falls back to Reload1 when
        ...
        // projected from its Quake-space origin into Godot and tinted redâ†’yellowâ†’green by VehLockStrength.
        ...
        // while a side-gun seat is unmanned (the QC test is `!AuxiliaryXhair[1/2].draw2d` â€” no gunner aux crosshair
        ...
        // (RSM_BOMB â†’ vCROSS_BURST, RSM_FLARE â†’ vCROSS_RAIN) plus, in bomb mode, a tracetoss-predicted bomb-impact
        // dropmark; the spiderbot draws a per-rocket-mode reticle (SBRM_VOLLY â†’ vCROSS_BURST, SBRM_GUIDE â†’
        // vCROSS_GUIDE, SBRM_ARTILLERY â†’ vCROSS_RAIN, spiderbot.qc vr_crosshair). Pure presentation; the mode is
    ...
    /// spectatee's entity slice (<c>SpectatingNetId</c> â†’ <c>TryGetRemoteState(...).VehicleView</c>, mirroring the
    ...
    private void UpdateVehicleHudRemote(VortexArena.Game.Hud.VehicleHud hud)
    {
        ...
        // Inactive (on foot / observing) â†’ the HUD_NORMAL exit case.
        if (!v.IsActive)
        {
            if (hud.InVehicle)
                hud.Exit();
        ...
        // straight onto the panel â€” no 0.01 * STAT(...) scaling like the host path.
        ...
        // precise aux lock crosshair can't be projected on the remote path â€” clear it (host-only nicety). The
        ...
        // tracetoss bomb dropmark is suppressed â€” the remote path draws the reticle only, never the green live mark).
    ...
    private static void FeedRemoteReticle(VortexArena.Game.Hud.VehicleHud hud, in VortexArena.Net.VehicleViewState v)
    {
        ...
        switch (v.VehKind)
        {
            case 2: // raptor â€” QC vr_crosshair: RSM_FLARE (2) â†’ vCROSS_RAIN; RSM_BOMB (1)/default â†’ vCROSS_BURST.
                hud.MainReticle = v.W2Mode == (int)RaptorMode.Flare
                    ? "gfx/vehicles/crosshair_rain"
                    : "gfx/vehicles/crosshair_burst";
                break;
            case 3: // spiderbot â€” SBRM_VOLLY â†’ vCROSS_BURST; SBRM_ARTILLERY â†’ vCROSS_RAIN; SBRM_GUIDE (default) â†’ vCROSS_GUIDE.
                ...
                break;
            case 4: // bumblebee pilot â€” the centered vCROSS_HEAL heal-gun pointer (QC bumblebee vr_crosshair).
                hud.MainReticle = "gfx/vehicles/crosshair_heal";
                break;
            default: // racer / bumblebee-gun gunner â€” no centered per-mode reticle.
                hud.MainReticle = "";
                break;
        }
    }
    ...
    /// centered reticle by the active rocket mode â€” SBRM_VOLLY â†’ vCROSS_BURST, SBRM_GUIDE â†’ vCROSS_GUIDE,
    ...
    private void FeedSpiderbotReticle(VortexArena.Game.Hud.VehicleHud hud, Entity veh)
    {
        ...
        // QC vr_crosshair: SBRM_VOLLY (1) â†’ vCROSS_BURST; SBRM_GUIDE (2) â†’ vCROSS_GUIDE; SBRM_ARTILLERY (3) â†’
    ...
    private void FeedRaptorReticle(VortexArena.Game.Hud.VehicleHud hud, Entity veh, Player p)
    {
        if (veh.VehicleDef is not Raptor)
        {
            ...
            // (gfx/vehicles/crosshair_heal) â€” the heal-gun aiming reticle. It is shown for the bumblebee body
            ...
            // Base â€” only the BEAM colour changes â€” so the pointer is unconditional. The shared
        ...
        // QC vr_crosshair: RSM_FLARE (2) â†’ vCROSS_RAIN; RSM_BOMB (1)/default â†’ vCROSS_BURST.
    ...
    /// <summary>QC bumblebee_gunner_frame reload tint: <c>'1 0 0' * reload1 + '0 1 0' * (1 - reload1)</c> â€” red while
    ...
    /// <summary>Drain the networked match clock (NetControl.MatchState â†’ ClientNet) into the TIMER panel each
    ...
    private void UpdateMatchClock()
    {
        ...
        // while observing, 2/4 keep showing). QC spectatee_status == -1 â†’ IsObserving (not following anyone).
    ...
    /// <see cref="VoteController"/> (GameWorld.Voting) carries it directly â€” Active / Display / yes-no tallies /
    ...
    /// there is no in-process world (no vote net channel yet) â†’ the panel stays blank (an honest gap).
    /// </summary>
    private void UpdateVotePanel()
    {
        ...
        v.Active = true; // set last so the fade-in only arms on the trueâ†’falseâ†’true transition
    ...
    private void UpdateMapVotePanel()
    {
        ...
        if (sig != _mapVoteSig)
        {
            ...
            for (int i = 0; i < cands.Count; i++)
            {
                ...
                // EVERY option, so stock ballots (dm/tdm/ca/ctf â€” which set neither cvar) showed the raw "dm"
        ...
        // The local player's own vote (QC mv_ownvote) + the decided winner slot â€” both normalized above from the
    ...
    /// <summary>A normalized map/gametype-vote ballot cell â€” the shared shape the map-vote panel renders, built
    ...
    /// This is the host analogue of the TE_CSQC_RACE net feed (QC race_SendTime) â€” the records are now kept
    ...
    private void UpdateRacePanels()
    {
        ...
        if (race is not null)
        {
            ...
            // QC RACE_NET_PENALTY_RACE / _QUALIFYING â†’ racetimer.qc penalty line: show "PENALTY: Ns (reason)" for the
            ...
            // QC race_SendTime â†’ the racetimer split feed: the most-recent checkpoint crossing (race_checkpoint /
            ...
            // QC race_SendStatus â†’ cl_race.qc HUD_Mod_Race medal flash: when the LOCAL racer files a new record
        ...
        else
        {
            ...
            // QC race_SendStatus â†’ cl_race.qc HUD_Mod_Race medal flash: when the LOCAL runner files a new record
            // (Cts.FinishStage stamps LastRecordTime), raise the medal once â€” identical mapping to the Race branch
            // (CTS forces g_race_qualifying=1, so its finish runs the same QC race_setTime â†’ race_SendStatus path).
    ...
    /// <summary>QC <c>count_ordinal</c> (common/util.qc): 1 â†’ "1st", 2 â†’ "2nd", 3 â†’ "3rd", 11..13 â†’ "Nth".</summary>
    ...
    /// there is no local Player so it stays null (the player-agnostic panels â€” centerprint/killfeed â€” still draw,
    ...
    /// the listen host â€” the CSQC model, where the HUD draws from replicated STATs, not server objects. Null on
    ...
    /// (<see cref="ClientNet.LocalState"/> â€” the powerups panel timers). Raw field writes, NEVER the hooked
    /// SetResource â€” the mirror is a render-side stats holder, not a sim actor, and must not fire mutator hooks.
    /// </summary>
    ...
    private void UpdateFullHudPlayer()
    {
        ...
        // by UpdateHudMirror â€” null until the handshake accepts). Either way the full skinned HUD renders.
        ...
        // below â€” this only sets the player-presence base state on a player change.
    ...
    /// crosshair entirely â€” and resets its smoothing state â€” whenever the player is not in a live first-person
    ...
    private void UpdateCrosshairGate()
    {
        ...
        //   STAT(HEALTH) <= 0           â†’ dead (LocalDeadNow covers host + remote)
        //   intermission == 2 / GAME_STOPPED â†’ MatchIntermission (the match-over scoreboard takes the screen)
        //   scoreboard_active           â†’ the scoreboard panel is up (+showscores / death / intermission)
        //   spectatee_status == -1      â†’ free-fly observer (IsObserving) â€” no own first-person aim to draw on
        bool suppressed =
            LocalDeadNow()
        ...
        // UpdateCamera â†’ FirstPersonView.UpdateView has already run this frame, so RenderedEyeQuake/
        // RenderedForwardQuake carry the final rendered eye/forward (derived from the already-predicted local pose â€”
        ...
        // Only the local player's own third-person counts â€” when spectating someone else (SpectatingNetId != 0) the
    ...
    private void UpdateInfoMessages()
    {
        ...
        // drawing for a SPECTATOR who has the scoreboard open (ENTCS_SPEC_IN_SCOREBOARD) â€” otherwise the panel
    ...
    /// new scoreboard frame has arrived it hydrates the panel rows from the wire (resolving netIdâ†’local via the
    ...
    private void UpdateScoreboard()
    {
        ...
        // key needed); at intermission==2 â€” when the MapVote panel owns the screen â€” it hides. We model
        ...
        // (_scoreboard is standalone â€” not _fullHud's discovered panel â€” so the owner has to bridge the two.)
        if (_fullHud is not null)
        ...
        // above never reaches it â€” its LiveFgAlpha is pinned. Fade it directly by the scoreboard level via Modulate
        // so the top-left minimap doesn't bleed through the scoreboard (QC: teamradar is MAINGAME-only). EXCEPT while
        // it's Maximized: the clickable spawn-select radar is itself shown while dead â€” exactly when the death
        // scoreboard is also up â€” and must stay fully visible to pick a spawn (force full alpha so a fade that began
        ...
        if (sb is not null && (!ReferenceEquals(sb, _lastFedScoreboard) || !ReferenceEquals(ms, _lastFedModeStatus)))
        {
            ...
            // scoreboard's rankings block is live in race modes (empty otherwise â†’ DrawRankings hides it). The local
    ...
    /// <summary>[T68] The local client's team for the shownames <c>sameteam</c> gate â€” the C# stand-in for the
    ...
    /// <summary>Feed + toggle the mod-icons panel (QC HUD_ModIcons_SetFunc â†’ gametype.m_modicons) from the
        ...
        if (panel.Mode == ModIconsPanel.ModIconsMode.None)
        {
            ...
            if ((raceGt is not null || ctsGt is not null) && me is not null && !_client!.IsObserving)
            {
                ...
                if (show2)
                {
                    ...
                    // QC race_SendStatus â†’ race_status / race_status_name / race_status_time: the medal flash.
    ...
    /// (<see cref="ScoreboardPanel.SetAccuracy"/>, registry-id â†’ hit% 0..100, -1 never fired) and the weapons
    /// panel's per-NetName grid (<see cref="WeaponsPanel.SetAccuracy"/>, NetName â†’ %). QC accuracy_byte decode:
    /// 0 â†’ never fired (-1 for the scoreboard's sentinel / omitted from the weapons map), 255 â†’ 100% (capped),
    /// else byteâˆ’1 = the percentage.
    /// </summary>
    ...
    /// every frame while the scoreboard is shown â€” not only when the row data changes.</summary>
    private void FeedMapStats()
    {
        // [T43] Feed the scoreboard map-stats row (QC monsters_setstatus â†’ STAT(MONSTERS_TOTAL/KILLED)).
    ...
    /// predict from the world origin until the first reconcile pulled it across â€” a visible snap.)
    /// </summary>
    ...
    // =====================================================================================
    //  Input sampling (WASD/mouse â†’ InputCommand) â€” mirrors PlayerController's gather
    // =====================================================================================
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        // it owns the mouse â€” cursor motion tracks the panel, a left click on a control point in Onslaught issues
        ...
        // WEAPON-SWITCH / RELOAD binds into a C2S impulse (the QC usercmd.impulse channel â€” the real way a human
        // switches weapons on the net path) and forwards everything else (kill/say/team/â€¦) to the shared
        ...
        // and while the chat prompt is open (DP key_dest = message swallows ALL input, mouse buttons included â€”
        ...
        // in-game menu) â€” DP's IN_Move: raw deltas sum into in_mouse_x/y over the frame; the application (the
        // m_accelerate/m_filter block + the m_yaw/m_pitch Ã— sensitivity scale) happens ONCE per render frame in
    ...
    /// A hardcoded 0.025 here once replaced m_yaw (DP 0.022) â€” +13.6% view turn per count vs Base at equal
    ...
    // the raw frame cadence â€” counts accumulate over the real previous pump interval but are displayed for
    ...
    // swipe stays exactly Î£(counts) â€” 1:1 aim is preserved; only the per-frame distribution shifts by
    // â‰¤Â±50% of one frame's counts. The ledger drains fast (50%/frame) when input stops so a swipe's tail
    ...
    /// <summary>
    /// QC client <c>MapVote_SendChoice(index)</c> â†’ <c>localcmd("impulse ", index+1)</c>: cast the local player's
    /// vote for the 0-based ballot option. On a listen server this routes through the in-process server's impulse
    /// command (the same path a console <c>impulse N</c> takes â€” <see cref="Commands.DispatchImpulse"/> â†’
    /// <see cref="MapVoting.CastVote"/>). On a pure remote client there is no server-side MapVoting object, so the
    /// cast rides the existing C2S impulse byte instead â€” <see cref="_pendingImpulse"/> is stamped and the next
    ...
    private void CastMapVote(int index)
    {
        ...
        // impulse byte (QC MapVote_SendChoice â†’ localcmd("impulse N")). The next InputCommand carries it to the
        // server, whose gated impulse path (Commands.DispatchImpulse) routes impulse 1..N to MapVoting.CastVote
        // while the vote runs â€” the same in-process call the listen path makes directly.
        if (_serverWorld is null)
        {
        ...
        if (me is null)
            return;
        // WS1 stage 3: world mutation from a main-thread input event â€” sim thread when threaded (inline when not).
        GameWorld w = _serverWorld;
        if (_server is { } srv)
            srv.RunOnSimThread(() => w.Commands.Execute($"impulse {index + 1}", isServerConsole: false, caller: me));
        else
            w.Commands.Execute($"impulse {index + 1}", isServerConsole: false, caller: me);
    }

    /// <summary>
    /// Run a one-shot console command issued by a bind on the NET path. Weapon-switch / reload binds are turned
    /// into a C2S impulse number (QC usercmd.impulse â€” the faithful way the weapon command rides the move
    ...
    /// effects/â€¦) forwards to the shared interpreter (<see cref="RunCommand"/>, wired by the Shell). Mirrors the
    ...
    private void RunBoundCommand(string command)
    {
        ...
        // QuickMenu (goal 8): the `quickmenu` bind (BindInput maps a key to "quickmenu") toggles the panel â€” the
        ...
        // Maximized radar (QC the `m` bind â†’ +hud_panel_radar_maximized â†’ cl_cmd hud radar 1): a toggle, like the
        // quickmenu. BindInput emits the press form `+hud_panel_radar_maximized` (and `hud radar 1` if bound directly);
        // both flip the maximized state. The release form (`-hud_panel_radar_maximized`) is a no-op â€” this is a press-
        ...
        if (imp != 0)
        {
            ...
            // best/by-id) â€” NOT drop (17) or reload (20) â€” and only when the switch could actually succeed
        ...
        // Not a weapon command â€” forward to the shared interpreter (kill/say/team/â€¦). Unknown commands there are
    ...
    /// (1..9 â†’ N, 0 â†’ 14), <c>weapnext</c>/<c>weapprev</c> (10/12), <c>weapon_last</c>/<c>weaplast</c> (11),
    ...
    {
        // weapon_group_N (the 1..9 keys via binds-xonotic.cfg) â†’ impulse N (group 0 â†’ 14).
        ...
        // weapon_byid_N â†’ 230+N (direct weapon by unique id).
    ...
    /// <summary>
    /// Whether a weapon-select impulse could actually change the weapon â€” the gate for the keypress-predicted
    /// holster (playtest #31). Base plays NO switch animation on a denied switch: <c>W_SwitchWeapon</c> leaves
    /// <c>m_switchweapon</c> untouched when <c>client_hasweapon(â€¦, andammo, complain)</c> fails (selection.qc:274),
    /// so the weaponsystem state machine never raises/drops â€” you only hear the "weapons/unavailable" denial
    /// (which the port's server path already plays). The old predict-always holster lowered the gun on EVERY
    /// select press and recovered on the grace timer â€” a phantom switch animation when nothing could switch.
    ...
    /// best test for any other usable weapon ("last"/"best" can rarely still deny with this true â€” e.g. best ==
    /// current â€” a small over-predict the holster grace recovers). Pure remote client (no
    /// <see cref="LocalServerPlayer"/>): keep the old predict-always behavior â€” the same graceful-degradation
    ...
    private bool SwitchCouldSucceedNow(int imp)
    {
        ...
        if (imp >= 230) // weapon_byid_N â€” exact target known
        {
            Weapon? w = WeaponOrder.WeaponByIdIndex(imp - 230);
            if (w is null)
                return false; // out of range â†’ QC WEP_Null no-op, nothing to animate
    ...
    /// <summary>A usable (owned + ammo, QC <c>client_hasweapon</c>) weapon other than the current one exists â€”
    ...
    /// <summary>The live look sensitivity (DP `sensitivity` cvar) folded with the base feel â€” the value the
    ...
    /// <summary>DP `m_yaw` (default 0.022 deg/count): the yaw half of the mouse scale. Unset/0 â†’ 0.022.</summary>
    ...
    /// <summary>DP `m_pitch` (default 0.022 deg/count, SIGNED â€” negative inverts the Y axis; the input
    /// dialog's "Invert aiming" writes Â±|value|). Unset/0 â†’ +0.022. Legacy configs hold Â±1 from the old
    /// sign-only invert convention (ClientSettings used to register "1") â€” any DP-implausible magnitude
    /// (â‰¥0.5) is read as sign Ã— 0.022 so an archived Â±1 doesn't give 45Ã— pitch.</summary>
    private static float MPitch()
    {
        float v = Api.Services is not null ? Api.Cvars.GetFloat("m_pitch") : 0f;
        if (v == 0f) return 0.022f;
        if (MathF.Abs(v) >= 0.5f) return v < 0f ? -0.022f : 0.022f; // legacy sign-only Â±1
        return v;
    }

    /// <summary>
    /// Sample one tick of live input into an <see cref="InputCommand"/> (the unit the client predicts on and
    /// the server applies). WASD â†’ Quake-space forward/side (normalised to Â±1; the move code rescales against
    /// sv_maxspeed), Space â†’ jump, Ctrl â†’ crouch, mouse buttons â†’ attack. The accumulated view angles ride
    ...
    /// </summary>
    /// <summary>True while the in-game minigame UI owns input â€” the Create/Join menu is open OR a minigame board
    ...
    /// true exactly on the tick the local player is predicted through a single-dest teleporter â€” snap the
    ...
    {
        // PREDICTED WARPZONE CROSSING (the seq-keyed one-shot pulse â€” see TriggerTouch.LastPredictedWarpSeq):
        // apply the view rotation IMMEDIATELY and RELATIVELY â€” `view = T(view)`, Base's
        // `setproperty(VF_CL_VIEWANGLES, WarpZone_TransformVAngles(this, getpropertyvec(VF_CL_VIEWANGLES)))`
        // (lib/warpzone/client.qc:141) â€” and rotate the pending input ring (DP CL_RotateMoves, builtin #638)
        // so post-warp reconcile replays use post-warp view angles.
        //
        // DISABLED BY DEFAULT (wz_predict_apply 0): on the LISTEN HOST this proved counterproductive â€” the
        ...
        // double-rotating the server state and fighting the reconcile â€” worse than the 1-2 frame authoritative
        ...
        if (_carrier is not null && _carrier.FixAngle)
        {
            ...
            // warpzone seam a replay from a post-warp base can spuriously re-cross a zone the server never did â€”
            // stamping a BACK-ROTATED facing (observed live: predicted 172 re-stamped right after the correct
            // authoritative 82, leaving the view at the entry yaw â€” the "angles still wrong" report). Any stamp
            // arriving within the window after a fixangle APPLY (predicted or authoritative) is such an echo of
            // the same crossing â€” discard it. A genuine rapid re-crossing inside the window still gets its
            ...
            // mouse delta in between â€” a visible "view fights me" snap-back on each crossing at low fps. The
    ...
    // The last APPLIED fixangle of either kind (wall-clock seconds) â€” the replay-echo discard window's anchor.
    ...
    // signal the view smoothing snaps on â€” see the faithfulSmoothing block).
    ...
    /// held), gated on the active weapon / alive / ammo â€” so sustained fire feels instant; the local player's own
    ...
    /// refire rate â€” the FX only replay as fast as the weapon actually fires. Either way each press edge sets a
    ...
    private void UpdateLocalFireFeedback(float dt)
    {
        ...
        // server, so the input stream is unaffected â€” only the local FX skip.
        ...
        if (_predictFire)
        {
            ...
            // is HELD with the latch armed and ATTACK_FINISHED has expired â€” NOT only on a fresh press. The old
            ...
            // real fire sound/flash are suppressed as "already predicted" â†’ a fully silent rocket.
            ...
            // edge, then one per `refire` s while held) â€” but ONLY once the weapon's persistent ready clock has
    ...
    /// predicted phantom muzzle-flash/fire-sound every refire while held (playtest #24) â€” for these weapons we
    /// predict only on the press edge. <c>guidestop 1</c> disables guiding â†’ continuous fire like a normal weapon,
    ...
    private bool PrimaryRefireRequiresRelease(int wid)
    {
        ...
        // NOT reliably reachable from the client here â€” _sharedCvars (client/menu store) returned a stale non-zero
    ...
    /// other weapon's secondary is a real shot. Listen-server resolves nothing special â€” pure name gate.
    /// </summary>
    private bool SecondaryFiresShot()
    {
        ...
        return net switch
        {
            ...
            // W_MuzzleFlash, no weapon_prepareattack â€” ATTACK_FINISHED is NOT bumped). Treating it as a shot
            // popped a phantom flash AND advanced the shared client ready clock, which then swallowed the
            // prediction of the NEXT primary rocket (the fireâ†’detonateâ†’fire combo went silent).
    ...
    /// (no active weapon, or a loop/grapple weapon absent from <see cref="WeaponFireSounds"/> â†’ its networked sound
    ...
    /// was &gt; ~1.5 frames ago â€” a fresh tap/burst or a frame hitch) start from now so the opening shot isn't
    ...
    /// does not apply g_weaponratefactor / haste â€” a rare, pre-existing approximation, same as before).</summary>
    ...
    /// <summary>Whether the active weapon has ammo to fire â€” else predicting would play phantom shots after the
    ...
    private bool HasAmmoNow()
    {
        ...
        // Without this the weapon-arena Devastator had ammo=False, which silently killed ALL fire prediction â€”
    ...
    /// mainâ†’sim mutation (inline when unthreaded).
    /// </summary>
    ...
    private InputCommand SampleInput()
    {
        ...
        // in-game menu), the console is open, or the minigame menu is up â€” those own the cursor. Focus handling
        ...
        if (active)
        {
            // Bind-driven button state (set from key events by BindInput â†’ BindTable). Move axes are normalised
            // to Â±1 here; the movement code rescales against sv_maxspeed. Jump/crouch both drive the up axis
            ...
            // PHYS_INPUT_BUTTON_HOOK (+hook): the offhand-fire button â€” grapple hook / offhand blaster / nade.
            ...
            // the last sampled command into THIS command â€” so a tap shorter than one input tick still fires â€”
        ...
        // spectatee â€” so suppress local movement (don't predict our own walk, which would fight the follow-cam),
        // but KEEP the buttons: +attack cycles the spectatee and +attack2 drops to free-fly on the server. (A
        // free-fly observer, spectatee_status == own id, still moves â€” server + client agree, no drift.)
        ...
        // map-load hitch's sudden batch-ack collapses the unacked window past the 32u reconcile snap threshold â†’
        ...
        // NO movement while frozen â€” zero the move axes + clear jump/crouch. WARMUP is freely playable, so it is NOT
        ...
        // clear the pending value so it rides exactly one InputCommand â€” the redundant input tail re-sends the
    ...
    // =====================================================================================
    //  Camera (driven by the SHARED FirstPersonView â€” same component PlayerController uses)
    // =====================================================================================
    ...
    /// steps â€” a bot re-aims only every AI think, a human's angles land at the snapshot rate â€” and the raw
    ...
    /// spectatee change and on any component jump &gt; 90Â° (respawn / teleport / warp crossing), so it cannot
    ...
    private NVec3 SmoothSpectateAngles(NVec3 target, float dt)
    {
        ...
        {
            _specAngSmoothed = target;   // discontinuity â€” snap, don't slew
            return target;
        }
    ...
    /// (Health&lt;=0 â€” the net path has no MaxHealth). FirstPersonView places + orients the camera with the proven
    /// AngleVectors â†’ Coords.ToGodot columns â†’ Basis(right, up, -forward) path (a negated-yaw Euler silently flips
    /// handedness), runs the death/event-chase pull-back, lerps the zoom, applies the *0.75 fov, and samples the
    /// eye contents at the FINAL render origin â€” exactly as on the local path. <paramref name="dt"/> 0 = a
    ...
    private void UpdateCamera(float dt = 0f)
    {
            ...
            && _client.SampleRemote(_client.SpectatingNetId, _client.LatestServerTime, out NVec3 specOrg, out NVec3 specAng))
        {
            ...
            // mode from chase_active just like the own-player path below â€” but anchored to the spectatee's pose.
            // chase_active == 0 keeps the faithful first-person follow (see what the spectated player sees).
            _view.Spectating = true;
            _view.CameraMode = CvarOr(Api.Cvars, "chase_active", 0f) != 0f
                ? Client.FirstPersonView.ChaseMode.Chase
                : Client.FirstPersonView.ChaseMode.None;
            // The camera sits inside the followed player in first person â€” hide that body exactly like one's
            ...
            var sst = new Client.FirstPersonView.ViewState
            {
                ...
                // See what the spectated player sees â€” through a smoother: a bot only re-aims every AI think
                ...
                IsDead = false,              // following a live player â€” no death-cam pullback
        ...
        _render?.SetViewedFirstPerson(0);   // not following â†’ every body draws again
        ...
        // still networks the velocity it died with, so over a void that's a large downward speed â€” extrapolating
        ...
        // (net path has no MaxHealth, so dead = Health<=0 â€” but ONLY after the first spawn, so the pre-spawn
        ...
        // `this.velocity += CSQCPlayer_GetPredictionErrorV()`). It is normally ~0; after a smoothed correction â€”
        // above all a damage-knockback shove (Reconciler force smoothing) â€” it ramps the rendered velocity from the
        ...
        // few units of the extrapolated eye â€” so the error smoother never engages. Render-only: it does not touch
        ...
        // frame-to-frame at any fps that isn't a multiple of 72 â€” so the eye gets shoved by velocity*_inputAccum in a
        ...
        // Set 0 to render the eye at the last simulated tic (no extrapolation) â€” an A/B isolation switch for the
        ...
        // (CSQCPlayer_ApplySmoothing â€” bound()-based stairsmoothz glide + viewheightavg eye-height blend, error
        ...
        //   cl_movement_errorcompensation       â€” strength; 0 = snap to truth (smoothing off), the stock toggle.
        //   cl_movement_errorcompensation_force_time â€” how long an explosion/blaster shove glides (port extension;
        ...
        if (faithfulSmoothing)
        {
            ...
            // smoothed origin Z feeds OriginQuake.Z and the blended view height feeds EyeHeightZ â€” the same shape
            ...
            // A teleport this tick snaps the glide instead of smoothing the cross-map jump â€” QC csqcmodel_teleported
            ...
            // â€” see PredictWarpzonesAmbient; without this pulse the stair smoother glides the height difference
        ...
        var st = new Client.FirstPersonView.ViewState
        {
            ...
            // only â€” _viewAngles (the aim sent to the server) stays unpunched so shots still land on the crosshair.
            ...
            // by default) and the fall-bob swing trigger (cl_bobfall 0.05 â€” ON in stock Xonotic: the landing dip).
            OnGround = _carrier?.OnGround ?? false,
        ...
        // host or remote), so reuse it as the camera's gate. The chase pivot is the seated origin â€” st.OriginQuake
    ...
    /// <summary>SUPERCONTENTS at the FINAL render origin this frame â€” read back from the shared view after
    ...
    // =====================================================================================
    //  Effect rendering (decoded EFF_NET_* â†’ particles)
    // =====================================================================================
    ...
    private void OnEffectReceived(ClientNet.EffectEvent e)
    {
        ...
        // (QC Send_Effect_ â†’ __pointparticles), which the renderer resolves through the effectinfo.txt catalog.
    ...
    /// <summary>A decoded positional sound (DP SV_StartSound) â†’ play it on the renderer. Routes by the wire flags:
    ...
    private void OnSoundReceived(ClientNet.SoundEvent e)
    {
        ...
        // unaffected (they don't match LocalNetId). A non-predicted weapon (loops/grapple) isn't in the set â†’ its
    ...
    /// <summary>Play a one-shot NON-spatial 2D cue (DP <c>sound(world, â€¦, ATTN_NONE)</c> / a CSQC local play) â€”
    /// e.g. SND_BLIND on a darkness-nade onset. Loads through the same VFS <see cref="AssetLoader.LoadSound"/> the
    /// HUD/announcer use, plays on the SFX bus, and self-frees when finished (no pooled emitter â€” these fire
    ...
    /// <c>cmd minigame list-sessions</c> reply populates the Join menu). Harmless for non-minigame prints â€” the
    ...
    private PlayerModel? ResolvePlayerModel(Entity e)
    {
        ...
        // Wave 1 (perf report Â§9.4): do NOT parse+build synchronously here. The first snapshot of a bot match
        // carries EVERY player at once, and building all ~25 MB skeletal models in that one frame was a
        // ~154 MB allocation burst â†’ a 100â€“150 ms blocking gen2 GC (the worst release-build hitch). Return a
        // placeholder shell immediately; the IQM+sidecar parse runs on the thread pool and the Godot build
        // lands under the streamer's per-frame budget â€” one model per frame, High priority (ahead of idle warm).
    ...
    private PlayerModel? StreamSkeletalInto(Entity e, string model, int skin, string shellName, bool forced)
    {
        ...
        {
            // No streamer (shouldn't happen on the live path) â€” fall back to the old synchronous load.
        ...
        _streamer.Request(
            // Off-thread: pure-C# parse (now incl. the animation library, Â§12.3-1). Boxed in a non-null wrapper
            // because the streamer drops null results silently â€” but a FAILED parse must still reach the main
            ...
            box =>
            {
                ...
                {
                    DeliverPlayerModel(pm, e, model, skin, null, forced);   // miss â†’ fall-back attach, as before
                    return;
                }
                // (Â§12.3-1) Stage the texture pipeline BEFORE the delivery: one streamer job per material
    ...
    /// <summary>Non-null off-thread wrapper for a (possibly failed) skeletal parse â€” see ResolvePlayerModel.</summary>
    ...
    /// <summary>
    /// Main-thread delivery of a streamed player-model parse (budgeted by the streamer â€” one heavy build per
    ...
    private void DeliverPlayerModel(PlayerModel pm, Entity e, string model, int skin, AssetLoader.SkeletalModelParse? parse, bool forced)
    {
        ...
        if (!GodotObject.IsInstanceValid(pm) || pm.IsQueuedForDeletion())
            return;
        if (e.IsFreed)
            return; // entity gone â€” OnEntityRemove tears the node (and shell) down
        ...
        // intentionally differs from a FORCED name â€” so it is skipped for forced deliveries (whose live-ness is the
        ...
        if (parse is null)
        {
            // Not an IQM (or unreadable): remember (so the rebuild's resolve doesn't re-stream), then fall back â€”
        ...
        if (!pm.Active)
        {
            // Parsed as IQM but no poseable skeleton â€” match the sync path's fall-through.
            ForgetSkeletal(model, skin, forced);
            _render.RebuildEntityModel(e);
            return;
        }
    ...
    /// checks BEFORE re-streaming â€” so the <see cref="ClientWorld.RebuildEntityModel"/> re-attach terminates. For a
    ...
    /// <see cref="AssetLoader.LoadModel"/> (FORCEMODEL parity â€” a forced MD3/DPM still wins over the entity's own
    /// model), or null on a genuine VFS miss (QC <c>fexists</c>-miss â†’ the client keeps the entity's own model).
    /// </summary>
    private Node3D? BuildForcedPlayerModel(Entity e, string model, int skin)
    {
        ...
        // the cached non-IQM forced node (FORCEMODEL parity) or null (true miss â†’ entity's own model). LoadModel is
    ...
    /// <see cref="GameWorld"/> constructor takes, so the server's spawn loop (QC the entity-lump â†’
    ...
    /// <summary>The VFS vpaths a configured map name is probed at (bare â†’ maps/, +.bsp) â€” mirrors GameDemo.</summary>
    ...
    /// <summary>Cvars the listen-server boot sequence (or the map's worldspawn) authors itself â€” a console override
    ...
    /// sv_gravity is owned by each map's worldspawn. NOTE: sv_spectate is deliberately NOT here anymore â€” the boot
    ...
    private void AddLight()
    {
        ...
        var env = new Godot.Environment
        {
            ...
            // ~lightmap_scale 2.0). A filmic curve (ACES/Filmic) crushes the mid-tones and shadows here â€” verified
            // on stormkeep â€” so it's left off. Switch to ToneMapper.Aces (TonemapWhite ~1.5) for a punchier,
            ...
            // muzzle/explosion flashes). Threshold ~1.0 keeps ordinary lit walls from washing out â€” this is
    ...
    private void LoadClientMapFromServer(string mapName, string gametype)
    {
        ...
        // agree with authority (render and collision MUST match â€” GameDemo.cs:134/181).
        _droppedSubmodels = GameMapView.ComputeDroppedSubmodels(bsp, _gametype);
        ...
        if (Api.Services is VortexArena.Engine.Simulation.EngineServices es)
        {
            ...
            // and is here so that when a snapshotâ†’facade mover mirror lands, a "*N" setmodel resolves its clip brushes.
        ...
        // build the full brush world a SECOND time on this connect frame (both uses are main-thread â€” safe to share).
        ...
        // â€” radar minimap image + world bounds (QC mi_min/mi_max), so the image and the blips align;
        if (_radar is not null)
        {
            _radar.MapName = _map;
        ...
        // â€” waypoint-sprite distance-fade ramp (QC waypointsprite_fadedistance = vlen(mi_scale));
        ...
        // â€” map music: SetupMusic ran with no map name, so resolve the cdtrack now. The MusicPlayer re-reads
        //   CdTrack each frame, so a late set starts the track cleanly. (trigger_music/target_music volumes stay
        //   listen-host-only â€” they are server entities and aren't networked.) Resolve order mirrors SetupMusic:
        //   the mapinfo cdtrack first, then â€” for legacy/Q3/Nexuiz-compat maps with no mapinfo â€” the worldspawn
        ...
        if (_musicPlayer is not null && _vfs is not null)
        {
            ...
            if (_vfs.Exists(mapinfoPath))
            {
                ...
                catch { /* unreadable mapinfo â€” fall through to worldspawn */ }
            }
        ...
        // â€” warpzone portal window/disc renderers (SetupCameraAndHud skipped them â€” no _mapRoot yet). The zone
        //   transforms aren't networked, so on a pure client these degrade to the dark-mirror placeholder rather
        //   than the live see-through render â€” but the portal FRAMES render instead of nothing. _camera is live
    ...
    // (the old hardcoded BotName table moved into BotPopulation's bots.txt fallback â€” T39)
