...
class Policy(nn.Module):
    ...
    # Upper bound on the view-delta log-std. exp(-1.386) = 0.25 of the action range, and the action range is
    # NeuralAction.MaxYawRate = 22 degrees per step, so this caps exploration noise at about 5.5 deg/step of
    # yaw and 3 of pitch.
    #
    # Unbounded, it does not stay put. A Gaussian's differential entropy is log(sigma) + constant, with no
    # upper limit, so the entropy bonus can always buy more of it and the optimiser will keep buying.
    # Measured on the stage-6 checkpoint: log_std had grown from its -0.5 initialisation to -0.092, which is
    # sigma 0.912 of the range -- 20.1 deg/step of yaw against a 22 deg/step clamp. At the 18 Hz decision
    # rate that is roughly 360 deg/s of random spin on every SAMPLED action.
    #
    # Which is what training runs on. The bench evaluates the distribution mean, where sigma does not
    # appear at all, and that gap is the one visible in every update line of every run: sampled arrivals
    # 2-4%, shipped 5-8%. The rollouts were generated by a spinning bot, so PPO spent every step of every
    # experiment learning from trajectories the shipped policy would never produce.
    #
    # Settable, because -1.386 is a guess that has not been priced. It was chosen to stop sigma running away
    # to 0.912 (20.1 deg/step of yaw, roughly 360 deg/s of random spin), which it demonstrably was. But the
    # initialisation is -0.5, so this caps exploration well BELOW where the network starts, and stage 1 --
    # "run and turn" -- regressed from 100% under the old code to about 48% under this. That is the obvious
    # suspect and it is cheap to test.
    LOG_STD_MAX = -1.386
    ...
    def distributions(self, logits: torch.Tensor) -> tuple[list[Categorical], Normal]:
        ...
        for _, size in layout.CATEGORICAL_HEADS:
            ...
            off += size
        mean = torch.tanh(logits[..., off : off + layout.N_CONTINUOUS])
        std = self.view_log_std.exp().expand_as(mean)
    ...
    def act(self, obs: torch.Tensor, deterministic: bool = False):
        ...
        logp = logp + (-0.5 * z * z - log_std - 0.5 * self._LOG_2PI).sum(-1)
    ...
    def evaluate(self, obs: torch.Tensor, wire: torch.Tensor):
        ...
        entropy = entropy + (log_std + 0.5 * (self._LOG_2PI + 1.0)).sum()
