...
- **godot-4.6.3-ppc64-dotnet-rid.patch** (2026-08-19) — adds `"ppc64" => "ppc64le"` to
  `DetermineRuntimeIdentifierArch` in `modules/mono/editor/GodotTools/GodotTools/Export/ExportPlugin.cs`,
  the switch that turns Godot's architecture name into a .NET runtime identifier. Without it every export
  targeting PowerPC dies inside GodotTools with `ArgumentOutOfRangeException("Unexpected architecture")`
  before a byte is written, which puts the platform out of reach of `--export-release` however well the
  engine itself compiles. The C++ half of the same exporter already knows the architecture —
  `platform/linuxbsd/export/export_plugin.cpp` maps ELF `e_machine` `0x0015` to `"ppc64"` and offers it in
  the `binary_format/architecture` enum — so this is one missing line rather than a port. The two names
  differ on purpose: Godot spells the architecture `ppc64` (its SConstruct aliases `ppc64le` to it, and
  the engine is little-endian only) and .NET spells the RID `ppc64le`, there being no big-endian PowerPC
  RID at all. **Editor-side only** — GodotTools ships in the editor, never in a game binary, so this patch
...
and it also builds the editor, which the workflow does only as a means to the C# glue:
...
./vx build-engine --arch ppc64 --install     # POWER, installed where this tree looks for it
```

It verifies the patch hashes first — seconds, before the multi-hour part — skips patches already applied
so a re-run is safe, and prints the `sha256`, `bytes` and lockfile snippet a pin needs at the end.

The same thing by hand, which is what the script runs. Note the editor must be built FIRST even when a
template is all you want: the C# glue is generated by RUNNING an editor binary, and no .NET template can
be linked without it.

```bash
git clone --depth 1 --branch 4.6.3-stable https://github.com/godotengine/godot.git godot-4.6.3-vortex
cd godot-4.6.3-vortex
for p in ../VortexArena/tools/engine-patches/*.patch; do git apply "$p"; done
scons platform=windows target=editor arch=x86_64 module_mono_enabled=yes cvtt_export_templates=yes d3d12=no -j24
./bin/godot.windows.editor.x86_64.mono.exe --headless --generate-mono-glue ./modules/mono/glue
python ./modules/mono/build_scripts/build_assemblies.py --godot-output-dir=./bin --godot-platform=windows
scons platform=windows target=template_release arch=x86_64 module_mono_enabled=yes cvtt_export_templates=yes d3d12=no -j24
# -> bin/godot.windows.template_release.x86_64.mono.exe (+ .console.exe wrapper)
```

The Linux and macOS legs differ only in `platform=`/`arch=` and in dropping `d3d12=no`; the workflow's
matrix carries the per-leg values, and `build-engine.sh` derives them from the host.
